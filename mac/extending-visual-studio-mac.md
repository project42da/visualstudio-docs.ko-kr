...
[연습](~/extending-visual-studio-mac-walkthrough.md) 항목에서는 *명령*을 사용하여 열려 있는 텍스트 문서에 날짜와 시간을 삽입하는 확장 패키지를 빌드하는 방법을 설명합니다.
...
확장/확장 지점 아키텍처를 통해 Mac용 Visual Studio 개발이 신속한 모듈식 개발로 유지됩니다. 수많은 확장 형식이 있으므로 이 문서에서는 [연습](~/extending-visual-studio-mac-walkthrough.md)에서 빌드된 확장 패키지에서 사용되는 확장 형식을 중심으로 살펴보겠습니다.

### <a name="command-extensions"></a>명령 확장

[연습](~/extending-visual-studio-mac-walkthrough.md)에서는 실행할 때마다 호출되는 메서드를 가리키는 확장인 명령 확장을 사용합니다. 명령 확장은 `/MonoDevelop/Ide/Commands` 확장 지점에 항목을 추가하여 정의합니다. 다음 코드를 사용하여 `Manifest.addin.xml`에서 확장을 정의했습니다.
...
편집 메뉴에서 명령으로 호출하기 위해 이 연습에서는 `/MonoDevelop/Ide/MainMenu/Edit` 확장 지점에 연결하는 CommandItem 확장을 만듭니다.
...
[연습](~/extending-visual-studio-mac-walkthrough.md)에서 자세히 설명된 확장 패키지는 Mac용 Visual Studio의 텍스트 편집기를 다루지만, 사용자 지정이 가능한 여러 영역 중 하나일 뿐입니다. 개발에 사용할 수 있는 영역의 범위에 대한 자세한 내용은 [확장 트리 참조](http://monodevelop.com/Developers/Articles/Extension_Tree_Reference) 및 [API 개요](http://monodevelop.com/Developers/Articles/API_Overview)를 참조하세요. 고급 확장 패키지를 빌드하는 경우 [개발자 문서](http://monodevelop.com/Developers/Articles)도 참조하세요. 다음은 사용자 지정이 가능한 영역의 부분 목록입니다.
...
> [!NOTE]
Mac용 Visual Studio용 확장 시나리오 개선을 위해 현재 작업 중입니다. 확장을 만들고 있으며 추가 도움 또는 정보가 필요하거나 피드백을 제공하려는 경우 mhutch@microsoft.com에 문의하세요.
