...
|**모든 아웃바운드 참조 표시**|선택된 네임스페이스의 모든 파일에 대해 다음 항목을 찾아서 강조 표시합니다.<br /><br /> -   `schemaLocation` 특성 없이 import 문에서 참조하는 모든 네임스페이스<br />-   import 문과 include 문의 `schemaLocation` 특성에 지정되어 있는 선택된 네임스페이스 이외의 다른 네임스페이스의 모든 파일|  
