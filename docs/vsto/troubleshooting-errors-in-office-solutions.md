...
### <a name="control-properties-are-lost-when-you-create-a-new-project-based-on-a-document-from-an-existing-project"></a>기존 프로젝트의 문서를 기반으로 하여 새 프로젝트를 만드는 경우 컨트롤 속성이 손실됩니다.  
...
### <a name="errors-when-you-create-an-excel-workbook-project-based-on-an-existing-workbook"></a>기존 통합 문서를 기반으로 하여 Excel 통합 문서 프로젝트를 만드는 경우 오류 발생  
...
### <a name="errors-in-upgraded-office-2003-document-level-projects-that-contain-windows-forms-controls"></a>Windows Forms 컨트롤을 포함하는 업그레이드된 Office 2003 문서 수준 프로젝트에서 오류 발생  
...
### <a name="controls-appear-as-black-rectangles-on-the-document-or-worksheet"></a>컨트롤이 문서 또는 워크시트에서 검은색 사각형으로 표시됩니다.  
...
### <a name="controls-on-a-word-template-are-not-visible-in-visual-studio"></a>Word 서식 파일의 컨트롤이 Visual Studio에서 표시되지 않습니다.  
...
### <a name="insert-clip-art-command-does-nothing-in-the-visual-studio-designer"></a>클립 아트 삽입 명령을 수행해도 Visual Studio 디자이너에서 아무 작업도 수행되지 않습니다.  
 Excel 또는 Word가 Visual Studio 디자이너에에서 열려 있는 클릭 하는 **클립 아트** 단추는 **그림** 리본 메뉴의 열리지 않고는 **클립 아트** 작업창입니다. 클립 아트를 추가하려면 Visual Studio 외부의 주 프로젝트 폴더에 있는 통합 문서 또는 문서의 복사본(\bin 폴더에 있는 복사본 아님)을 열고 클립 아트를 추가한 다음 통합 문서 또는 문서를 저장해야 합니다.  
  
##  <a name="code"></a> 코드 작성  
 Office 프로젝트에서 코드를 작성할 때 다음과 같은 오류가 발생할 수 있습니다.  
  
### <a name="some-events-of-office-objects-are-not-accessible-when-using-c"></a>C#을 사용하는 경우 Office 개체의 일부 이벤트에 액세스할 수 없습니다.  
 ...
 Office Pia의 이벤트 인터페이스에 대 한 자세한 내용은 참조 [의 클래스 및 인터페이스 개요 Office 주 Interop 어셈블리에서](http://msdn.microsoft.com/en-us/da92dc3c-8209-44de-8095-a843659368d5)합니다.  
  
### <a name="cannot-reference-office-pia-classes-in-projects-that-target-the-includenetv40shortsharepointincludesnet-v40-short-mdmd-or-the-includenetv45vstoincludesnet-v45-mdmd"></a>[!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] 또는 [!INCLUDE[net_v45](../vsto/includes/net-v45-md.md)]를 대상으로 하는 프로젝트에서는 Office PIA 클래스를 참조할 수 없습니다.  
...
```  
 ...
 [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] 또는 [!INCLUDE[net_v45](../vsto/includes/net-v45-md.md)]를 대상으로 하는 프로젝트는 기본적으로 Office PIA의 모든 interop 형식을 자동으로 포함합니다. 이 컴파일 오류는 포함된 interop 형식 기능이 클래스가 아니라 인터페이스에서만 작동하기 때문에 발생합니다. Office Pia의 인터페이스 및 클래스에 대 한 자세한 내용은 참조 [의 클래스 및 인터페이스 개요 Office 주 Interop 어셈블리에서](http://go.microsoft.com/fwlink/?LinkId=189592)합니다. Office 프로젝트에 포함 된 interop 형식 기능에 대 한 자세한 내용은 참조 [솔루션 디자인 및 만들기 Office](../vsto/designing-and-creating-office-solutions.md)합니다.  
  
### <a name="references-to-office-classes-are-not-recognized"></a>Office 클래스에 대한 참조를 인식할 수 없습니다.  
...
### <a name="document-level-projects-created-at-unc-network-locations-do-not-run-from-visual-studio"></a>UNC 네트워크 위치에서 만든 문서 수준 프로젝트가 Visual Studio에서 실행되지 않습니다.  
...
### <a name="threads-are-not-stopped-correctly-after-debugging"></a>디버그 후 스레드가 제대로 중지되지 않습니다.  
...
### <a name="cannot-run-or-debug-any-office-solution-on-the-development-computer"></a>개발 컴퓨터에서 Office 솔루션을 실행 또는 디버그할 수 없음  
 개발 컴퓨터에서 Office 프로젝트를 실행 또는 개발할 수 없는 경우 다음과 같은 오류 메시지가 표시될 수 있습니다.  
  
 “응용 프로그램 도메인을 만들 수 없으므로 사용자 지정을 로드할 수 없습니다.”  
  
 Visual Studio는 Office 솔루션을 로드하기 전에 .NET Framework 어셈블리 로더인 Fusion을 사용하여 어셈블리를 캐시합니다. Visual Studio가 Fusion 캐시에 쓸 수 있는지 확인한 후 다시 시도합니다. 자세한 내용은 참조 [어셈블리 섀도 복사](/dotnet/framework/app-domains/shadow-copy-assemblies)합니다.  
  
### <a name="error-when-stopping-the-debugger-in-a-document-level-project-after-using-edit-and-continue"></a>편집하며 계속하기를 사용한 후 문서 수준 프로젝트에서 디버거를 중지하는 경우 오류 발생  
