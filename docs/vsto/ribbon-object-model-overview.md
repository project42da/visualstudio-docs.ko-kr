...
### <a name="common-tasks-using-the-properties-of-ribbon-controls"></a>리본 컨트롤의 속성을 사용 하 여 일반 작업  
 ...
 경우에 따라 속성 또는 컨트롤 동적 메뉴에 추가 된 후 리본 메뉴를 로드 한 후 읽기 전용으로 설정 됩니다. 자세한 내용은 참조 [되는 읽기 전용 속성 설정](#SettingReadOnlyProperties)합니다.  
...
-   프로젝트 메서드에 `ThisAddin`, `ThisWorkbook`, 또는 `ThisDocument` 프로젝트의 클래스입니다.  
  
 동적 메뉴에는 몇 가지 예외가 제공합니다. 새 컨트롤을 만들 하 고, 해당 속성을 설정 하 고, 다음 실행 시 리본 메뉴가 포함 된 로드 된 후에 동적 메뉴에 추가할 수 있습니다.  
  
 언제 든 지 동적 메뉴에 추가 하는 컨트롤의 속성을 설정할 수 있습니다.  
  
 자세한 내용은 참조 [되는 읽기 전용 속성](#ReadOnlyProperties)합니다.  
  
### <a name="setting-properties-in-the-constructor-of-the-ribbon"></a>리본 메뉴의 생성자에서 속성을 설정합니다.  
...
### <a name="setting-properties-in-the-createribbonextensibilityobject-method"></a>프로젝트 메서드에서 속성 설정  
 속성을 설정할 수는 `Ribbon` 제어에서 프로젝트 메서드를 재정의 하는 경우는 `ThisAddin`, `ThisWorkbook`, 또는 `ThisDocument` 프로젝트의 클래스입니다. 프로젝트 방법에 대 한 자세한 내용은 참조 [리본 개요](../vsto/ribbon-overview.md)합니다.  
  
 프로젝트 메서드에 리본 속성을 설정 하는 다음 예제는 `ThisWorkbook` 는 Excel 통합 문서 프로젝트의 클래스.  
...
### <a name="setting-properties-for-ribbons-that-appear-in-outlook-inspectors"></a>Outlook 검사기에 나타나는 리본 메뉴에 대 한 속성을 설정 합니다.  
...
> [!NOTE]  
>  확인는 **이름** Outlook 리본 메뉴에 추가 하는 각 컨트롤에 대 한 속성이 설정 되어 있습니다. 런타임 시 Outlook 리본 메뉴에 컨트롤을 추가 하면 코드에서이 속성을 설정 해야 합니다. 디자인 타임에 Outlook 리본 메뉴에 컨트롤을 추가 하면 Name 속성은 자동으로 설정 됩니다.  
