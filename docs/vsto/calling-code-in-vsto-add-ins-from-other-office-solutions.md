...
dev_langs: 
  ...
  - "CSharp"
helpviewer_keywords: 
  - "VBA[Visual Studio에서 Office 개발], 응용 프로그램 수준 추가 기능에서 코드 호출"
  - "응용 프로그램 수준 추가 기능[Visual Studio에서 Office 개발], 다른 솔루션에서 코드 호출"
  - "추가 기능 코드 호출"
  - "추가 기능[Visual Studio에서 Office 개발], 다른 솔루션에서 코드 호출"
  - "상호 운용성[Visual Studio에서 Office 개발]"
  - "VBA에서 코드 호출"
...
---
# 다른 Office 솔루션에서 VSTO 추가 기능의 코드 호출
...
### 다른 솔루션에 노출하려는 클래스 정의  
 최소한, 노출하려는 클래스는 public 클래스여야 하며, <xref:System.Runtime.InteropServices.ComVisibleAttribute> 특성이 **true**로 설정되어 있어야 하고, [IDispatch](http://msdn.microsoft.com/ko-kr/ebbff4bc-36b2-4861-9efa-ffa45e013eb5) 인터페이스를 노출해야 합니다.  
  
 [IDispatch](http://msdn.microsoft.com/ko-kr/ebbff4bc-36b2-4861-9efa-ffa45e013eb5) 인터페이스를 노출하는 데 권장되는 방식은 다음 단계를 수행하는 것입니다.  
...
2.  <xref:System.Runtime.InteropServices.ComVisibleAttribute> 특성을 이 인터페이스에 적용하고 이 특성을 **true**로 설정합니다.  
  
3.  이 인터페이스에 구현할 클래스를 수정합니다.  
  
4.  <xref:System.Runtime.InteropServices.ClassInterfaceAttribute> 특성을 클래스에 적용하고 이 특성을 None 열거형의 <xref:System.Runtime.InteropServices.ClassInterfaceType> 값으로 설정합니다.  
...
5.  클래스를 OOP\(Out\-of\-Process\) 클라이언트에 노출하려면 다음을 수행해야 할 수 있습니다.  
 ...
 다음 코드 예제에서는 다른 솔루션에서 호출할 수 있는 `AddInUtilities` 메서드와 `ImportData` 클래스를 보여 줍니다. 더 큰 규모의 연습 컨텍스트에서 이 코드를 확인하려면 [연습: VBA에서 VSTO 추가 기능의 코드 호출](../vsto/walkthrough-calling-code-in-a-vsto-add-in-from-vba.md)을 참조하세요.  
...
### VBA에 클래스 노출  
 ...
 아니면 특성을 <xref:System.Runtime.InteropServices.ClassInterfaceAttribute> 열거형의 AutoDispatch 또는 AutoDual 값으로 설정하여 <xref:System.Runtime.InteropServices.ClassInterfaceType>[IDispatch](http://msdn.microsoft.com/ko-kr/ebbff4bc-36b2-4861-9efa-ffa45e013eb5) 인터페이스를 노출할 수도 있습니다. 이렇게 하는 경우 별도의 인터페이스에서 메서드를 선언할 필요가 없습니다. 하지만 VBA 코드는 <xref:System.Object> 같은 기본 클래스에서 가져오는 메서드를 포함하여 클래스의 모든 public 및 비정적 메서드를 호출할 수 있습니다. 하지만 초기 바인딩을 사용하는 OOP\(Out\-of\-Process\) 클라이언트는 클래스를 호출할 수 없습니다.  
...
###  <a name="outofproc"></a> OOP\(Out\-of\-Process\) 클라이언트에 클래스 노출  
 ...
 Office 솔루션에서 스레드를 사용하는 방법에 대한 자세한 내용은 [Office의 스레딩 지원](../vsto/threading-support-in-office.md)을 참조하세요.  
  
### RequestComAddInAutomationService 매서드 재정의  
 다음 코드 예제에서는 VSTO 추가 기능의 `ThisAddIn` 클래스에서 <xref:Microsoft.Office.Tools.AddInBase.RequestComAddInAutomationService%2A>를 재정의하는 방법을 보여 줍니다. 이 예제에서는 다른 솔루션에 노출하려는 `AddInUtilities`라는 클래스를 정의했다고 가정합니다. 더 큰 규모의 연습 컨텍스트에서 이 코드를 확인하려면 [연습: VBA에서 VSTO 추가 기능의 코드 호출](../vsto/walkthrough-calling-code-in-a-vsto-add-in-from-vba.md)을 참조하세요.  
 ...
 VSTO 추가 기능이 로드되면 [!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)]에서 <xref:Microsoft.Office.Tools.AddInBase.RequestComAddInAutomationService%2A> 메서드를 호출합니다. 런타임에서는 VSTO 추가 기능을 나타내는 <xref:Microsoft.Office.Core.COMAddIn> 개체의 <xref:Microsoft.Office.Core.COMAddIn.Object%2A> 속성에 반환된 개체를 할당합니다. 이 <xref:Microsoft.Office.Core.COMAddIn> 개체는 다른 Office 솔루션은 물론, Office를 자동화하는 솔루션에서 사용할 수 있습니다.  
...
### VBA 솔루션에서 개체 액세스  
 다음 코드 예제에서는 VBA를 사용하여 VSTO 추가 기능을 통해 노출되는 메서드를 호출하는 방법을 보여 줍니다. 이 VBA 매크로는 **ExcelImportData**라는 VSTO 추가 기능에 정의된 `ImportData`라는 메서드를 호출합니다. 더 큰 규모의 연습 컨텍스트에서 이 코드를 확인하려면 [연습: VBA에서 VSTO 추가 기능의 코드 호출](../vsto/walkthrough-calling-code-in-a-vsto-add-in-from-vba.md)을 참조하세요.  
