...
| 프로젝트 형식 | Support(지원) |
| --- | --- |
| .NET Core 프로젝트(.xproj) | Visual Studio 2015에서 .xproj 프로젝트 파일을 포함하는 미리 보기 도구를 사용하여 만든 프로젝트입니다. Visual Studio 2017에서 .xproj 파일을 열면 파일을 .csproj 형식으로 마이그레이션(.xproj 파일의 백업이 생성됨)하라는 메시지가 표시됩니다. .NET Core 프로젝트에 대한 이 .csproj 형식은 VS2015 이전 버전에서 지원되지 않습니다.  .xproj 형식은 .csproj로 마이그레이션해야 만 Visual Studio 2017에서 지원됩니다. |
