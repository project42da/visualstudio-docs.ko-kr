---
title: "코딩된 UI 테스트를 사용하여 Windows Phone 8.1 앱 테스트 | Microsoft Docs"
ms.custom: ""
ms.date: "12/03/2016"
...
manager: "douge"
---
# 코딩된 UI 테스트를 사용하여 Windows Phone 8.1 앱 테스트
[!INCLUDE[vs2017banner](../code-quality/includes/vs2017banner.md)]

코딩된 UI 테스트를 사용하여 Windows Phone 앱을 테스트합니다.  
  
## 간단한 Windows Phone 앱 만들기  
  
1.  Visual C\# 또는 Visual Basic 템플릿을 사용하여 빈 Windows Phone 앱을 위한 새 프로젝트를 만듭니다.  
  
     ![새 Windows Phone 앱 만들기](../test/media/cuit_phone_app_newproject.png "CUIT\_Phone\_App\_NewProject")  
  
2.  솔루션 탐색기에서 MainPage.xaml을 엽니다. 도구 상자에서 단추 컨트롤과 textbox 컨트롤을 디자인 화면으로 끕니다.  
  
     ![MainPage.xaml에 컨트롤 추가](../test/media/cuit_phone_app_addcontrols.png "CUIT\_Phone\_App\_AddControls")  
  
3.  속성 창에서 단추 컨트롤의 이름을 지정합니다.  
  
     ![단추 컨트롤 이름 지정](../test/media/cuit_phone_namebutton.png "CUIT\_Phone\_NameButton")  
  
4.  textbox 컨트롤의 이름을 지정합니다.  
  
     ![텍스트 상자 컨트롤 이름 지정](../test/media/cuit_phone_nametesxtbox.png "CUIT\_Phone\_NameTesxtBox")  
...
6.  F5 키를 눌러 에뮬레이터에서 Windows Phone 앱을 실행하고 제대로 작동하는지 확인합니다.  
     ...
     ![Windows Phone 앱 실행](../test/media/cuit_phone_runapp.png "CUIt\_Phone\_RunApp")  
...
1.  코딩된 UI 테스트에서 앱의 컨트롤을 매핑하려면 앱을 배포해야 합니다.  
     ...
     ![Windows Phone 앱 배포](../test/media/cuit_phone_deploy.png "CUIT\_Phone\_Deploy")  
  
     에뮬레이터를 시작합니다. 이제 앱을 테스트할 수 있습니다.  
  
     ![에뮬레이터에 배포된 앱](../test/media/cuit_phone_deployed.png "CUIT\_Phone\_Deployed")  
  
     코딩된 UI 테스트를 만드는 동안 에뮬레이터를 계속해서 실행합니다.  
  
## Windows Phone 앱용 코딩된 UI 테스트 만들기  
  
1.  Windows Phone 앱을 사용하여 솔루션에 새로 코딩된 UI 테스트 프로젝트를 추가합니다.  
  
     ![새로 코딩된 Windows Phone용 UI 테스트 만들기](../test/media/cuit_phone_newproject.png "CUIT\_Phone\_NewProject")  
  
2.  십자선 도구를 사용하여 UI 맵을 편집하도록 선택합니다.  
  
     ![십자선 도구를 사용하여 코딩된 UI 테스트를 생성합니다.](../test/media/cuit_phone_howgencodedialog.png "CUIT\_Phone\_HowGenCodeDialog")  
  
3.  십자선 도구를 사용하여 앱을 선택한 다음 나중에 테스트에서 앱을 시작하는 데 사용하게 될 앱의 **AutomationId** 속성 값을 복사합니다.  
  
     ![앱의 AutomationId 값 복사](../test/media/cuit_phone_getautomationid.png "CUIT\_Phone\_GetAutomationId")  
  
4.  에뮬레이터에서 앱을 시작하고 십자선 도구를 사용하여 단추 컨트롤을 선택합니다. 그런 다음 UI 컨트롤 맵에 단추 컨트롤을 추가합니다.  
  
     ![컨트롤 매핑에 십자선 도구를 사용합니다.](../test/media/cuit_phone_mapbuttoncontrol.png "CUIT\_Phone\_MapButtonControl")  
  
5.  UI 컨트롤 맵에 textbox 컨트롤을 추가하려면 이전 단계를 반복합니다.  
  
     ![십자선 도구 사용 및 텍스트 상자 컨트롤 매핑](../test/media/cuit_phone_maptextboxcontrol.png "CUIT\_Phone\_MapTextBoxControl")  
  
6.  코드를 생성하여 UI 컨트롤 맵 변경을 위한 코드를 작성합니다.  
  
     ![작성기에서 코드 생성](../test/media/cuit_phone_generatecode.png "CUIT\_Phone\_GenerateCode")  
  
7.  십자선 도구를 사용하여 textbox 컨트롤을 선택한 다음 **텍스트** 속성을 선택합니다.  
  
     ![Text 속성 선택](../test/media/cuit_phone_textproperty.png "CUIT\_Phone\_TextProperty")  
  
8.  어설션을 추가합니다. 이 어설션은 값이 올바른지 확인하는 테스트에 사용됩니다.  
  
     ![테스트에 어설션 추가](../test/media/cuit_phone_addassertion.png "CUIT\_Phone\_AddAssertion")  
  
9. assert 메서드에 대한 코드를 추가하고 생성합니다.  
  
     ![어설션에 대한 코드 생성](../test/media/cuit_phone_generatecodeassertion.png "CUIT\_Phone\_GenerateCodeAssertion")  
...
1.  테스트를 빌드한 다음 테스트 탐색기를 사용하여 테스트를 실행합니다.  
     ...
     ![테스트 탐색기를 사용하여 테스트 빌드 및 실행](../test/media/cuit_phone_runtestexplorer.png "CUIT\_Phone\_RunTestExplorer")  
  
     Windows Phone 앱이 시작되고 단추 탭하기 작업이 완료되며 assert 메서드를 사용하여 텍스트 상자의 텍스트 속성이 채워지고 확인됩니다.  
  
     ![Winodws Phone 테스트 실행](../test/media/cuit_phone_runtestexplorerrunning.png "CUIT\_Phone\_RunTestExplorerRunning")  
  
     테스트를 마치면 테스트 탐색기에서 테스트를 통과했음을 확인합니다.  
  
     ![테스트 탐색기 결과](../test/media/cuit_phone_runtestexplorerresults.png "CUIT\_Phone\_RunTestExplorerResults")  
...
### Q: UI 컨트롤을 매핑하려면 에뮬레이터에서 Windows Phone 앱을 배포해야 하나요?  
 ...
 **A**: 두 옵션 모두 지원됩니다. 에뮬레이터 유형을 변경하거나 장치 도구 모음에서 장치를 선택하여 테스트 실행 대상을 선택합니다. 장치를 선택하면 컴퓨터의 USB 포트 중 하나에 Phone Blue 장치를 연결해야 합니다.  
 ...
 ![에뮬레이터 버전 또는 물리적 장치 선택](../test/media/cuit_phone_testtarget.png "CUIT\_Phone\_TestTarget")  
  
### Q: 코딩된 UI 테스트용으로 코드 생성 대화 상자에 나의 코딩된 UI 테스트를 기록할 수 있는 옵션이 표시되지 않는 이유는 무엇인가요?  
 **A**: Windows Phone 앱의 경우 기록 옵션이 지원되지 않습니다.  
  
### Q: WinJS, Silverlight 또는 HTML5를 기반으로 Windows Phone 앱용 코딩된 UI 테스트를 만들 수 있나요?  
 **A**: 아니요, XAML 기반 앱만 지원됩니다.  
  
### Q: Windows 8.1 또는 Windows 10이 실행되고 있지 않은 시스템에서 Windows Phone 앱을 위한 코딩된 UI 테스트를 만들 수 있나요?  
...
### Q: 에뮬레이터 외부에 있는 컨트롤을 선택할 수 있나요?  
 **A**: 아니요, 빌더가 이러한 컨트롤은 검색하지 않습니다.  
  
### Q: 코딩된 UI 테스트 빌더를 사용하여 실제 휴대폰 장치에 컨트롤을 매핑할 수 있나요?  
...
### Q: Windows Phone 앱의 명령줄에서 코딩된 UI 테스트를 실행할 수 있나요?  
 **A**: 예, runsettings 파일을 사용하여 테스트를 실행할 대상 장치를 지정할 수 있습니다. 예를 들면 다음과 같습니다.  
...
### Q: XAML 기반 Windows 스토어 앱과 Windows Phone 앱용 코딩된 UI 테스트 간의 차이점은 무엇인가요?  
...
|데이터 기반 테스트|외부 데이터 소스 사용 및 테스트 메서드에 DataSource 특성 사용에 대한 자세한 내용은 [데이터 기반 테스트](../test/creating-a-data-driven-coded-ui-test.md)를 참조하세요.|데이터는 테스트 메서드에 DataRow 특성을 사용하여 인라인으로 지정됩니다. 이 항목의 [Windows Phone 앱에서 데이터 기반의 코딩된 UI 테스트 사용](#TestingPhoneAppsCodedUI_DataDriven)을 참조하세요.|  
 ...
 Windows 스토어 앱용 코딩된 UI 테스트에 대한 자세한 내용은 [코딩된 UI 테스트를 사용하여 Windows 스토어 8.1 앱 테스트](../test/test-windows-store-8-1-apps-with-coded-ui-tests.md)를 참조하세요.  
