...
### <a name="to-customize-run-settings-in-a-command-line-test"></a>명령줄 테스트에서 실행 설정을 사용자 지정하려면  
 명령줄에서 테스트를 실행하려면 vstest.console.exe를 사용합니다. 설정 파일은 이 유틸리티의 매개 변수입니다. 자세한 내용은 [명령줄에서 VSTest.console 사용](/devops-test-docs/test/using-vstest-console-from-the-command-line)을 참조하세요.  
