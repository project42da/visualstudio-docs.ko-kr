...
| 영구적으로 숨겨진 모든 대화 상자 다시 설정(단추) | 해당 없음 | 여러 대화 상자에서 **이 대화 상자를 다시 표시 안 함** 등의 옵션을 제공합니다. 이러한 옵션의 선택을 취소하고 대화 상자를 다시 표시하려면 이 단추를 사용합니다. |
...
| 옵션 | 기본 | 설명 |
| --- | --- | --- |
| 스크립트 | 해당 없음 | 모든 환경에 대한 대화형 창에 적용할 시작 스크립트에 대한 일반 폴더를 지정합니다. [시작 스크립트](python-environments.md#startup-scripts)를 참조하세요. 그러나 이 기능은 현재 작동하지 않습니다. |
