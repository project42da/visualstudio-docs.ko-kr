...
1. 테스트 탐색기는 테스트에 대한 프로젝트를 검색하고 아래와 같이 표시합니다. 테스트를 두 번 클릭하면 해당 소스 파일이 열립니다.
    ...
    ![기본 test_A를 보여 주는 테스트 탐색기](~/docs/python/media/unit-test-A.png)

1. 프로젝트에 더 많은 테스트를 추가함에 따라 도구 모음의 그룹화 방법 메뉴를 사용하여 테스트 탐색기에서 보기를 구성할 수 있습니다.

    ![테스트 탐색기 그룹화 방법 도구 모음 메뉴](~/docs/python/media/unit-test-group-menu.png)
...
- 성공한 테스트에는 녹색 릭과 테스트를 실행하는 데 소요된 시간이 표시됩니다.
    ...
    ![test_A 성공 상태](~/docs/python/media/unit-test-A-pass.png)

- 실패한 테스트에는 빨간색 십자 표시와 콘솔 출력 및 테스트 실행의 `unittest` 출력을 보여 주는 **출력** 링크가 표시됩니다.

    ![test_A 실패 상태](~/docs/python/media/unit-test-A-fail.png)

    ![test_A 실패 및 이유](~/docs/python/media/unit-test-A-fail-reason.png)
...
![테스트 디버깅](~/docs/python/media/unit-test-debugging.png)
