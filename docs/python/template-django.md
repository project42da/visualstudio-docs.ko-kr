...
- 템플릿에 전달된 컨텍스트 변수:
    ...
    ![컨텍스트 변수에 대한 IntelliSense](~/docs/python/media/template-django-intellisense.png)

- 기본 제공 및 사용자 정의 항목에 대한 태깅 및 필터링

    ![태그 및 필터에 대한 IntelliSense](~/docs/python/media/template-django-intellisense-filter.png)
...
![중단점](~/docs/python/media/template-django-debugging.png)
...
- **Django Shell 열기...**: 모델을 조작할 수 있는 응용 프로그램 컨텍스트에서 셸을 엽니다.
    ...
    ![콘솔](~/docs/python/media/template-django-console-shell.png)

- **Django Sync DB**: 대화식 창에서 `manage.py syncdb`를 실행합니다.

    ![콘솔](~/docs/python/media/template-django-console-sync-db.png)
...
- **Collect Static**: `manage.py collectstatic --noinput`을 실행하여 `settings.py`의 `STATIC_ROOT`에 지정된 경로에 모든 정적 파일을 복사합니다. [Microsoft Azure에 게시](template-web.md#publishing-to-azure-app-service)할 때 정적 파일은 게시 작업의 일부로 자동으로 수집됩니다.
    ...
    ![콘솔](~/docs/python/media/template-django-console-collect-static.png)

- **Validate**: `manage.py validate`를 실행하여 `settings.py`의 `INSTALLED_APPS`에 지정된 설치된 모델에서 모든 유효성 검사 오류를 보고합니다.

    ![콘솔](~/docs/python/media/template-django-console-validate.png)
