...
- [App Service에서 Python 구성](#configure-python-on-azure-app-service)
...
- [App Service에서 원격 디버깅](#remote-debugging-on-azure-app-service)
...
> [!Tip]
> Azure에서 사용자 계정을 확인하기 위해 신용 카드를 요청하나, 카드에 요금이 부과되지는 않습니다. 추가 요금이 발생하지 않는 무료 크레딧과 동일한 [지출 한도](/azure/billing/billing-spending-limit)를 설정할 수도 있습니다. 또한 Azure는 다음 섹션에 설명된 대로 간단한 테스트 앱에 이상적인 무료 App Service 계획 계층을 제공합니다.
...
이러한 파일이 서버에 배포되면(Visual Studio 2015에서는 자동으로, Visual Studio 2017에서는 다음 게시 단계에서) [Azure 원격 디버깅](debugging-azure-remote.md)에 대한 지침을 수행할 수 있습니다.
