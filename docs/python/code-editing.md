...
> [!Tip]
> **도구 > 옵션 > 텍스트 편집기 > Python > 고급**을 통해 완성 동작을 구성할 수 있습니다. 여기서 **검색 문자열 기준 필터 목록**은 입력 시 완성 제안 필터링을 적용하며(기본적으로 선택한 상태), **멤버 완성에서 멤버 교집합 표시**는 가능한 모든 형식에서 지원되는 완성만 표시합니다(기본적으로 선택 취소됨). [옵션 - 완성 결과](options.md#completion-results)를 참조하세요.
...
> [!Tip]
> 코드 색 지정을 사용하지 않으려면 **도구 > 옵션 > 텍스트 편집기 > Python > 고급**으로 이동하여 **기타 옵션 > 형식별 색 이름**을 선택 취소합니다. [옵션 - 기타 옵션](options.md#miscellaneous-options)을 참조하세요.
