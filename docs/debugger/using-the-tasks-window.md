...
|**Status**|현재 상태 (예약 된, 활성, 차단, 교착 상태, 대기, 또는 완료 된) 작업입니다. 예약된 작업은 아직 실행되지 않은 작업이므로 호출 스택, 할당된 스레드 또는 관련 작업이 없습니다.<br /><br /> 활성 작업은 디버거를 시작하기 전에 코드를 실행 중이던 작업입니다.<br /><br /> 대기 중 또는 차단 된 작업은 이벤트 신호를 받거나, 잠금이 해제 될 또는 다른 작업이 완료 되기를 기다리고 있어서 차단 된 작업입니다.<br /><br /> 교차 상태의 작업은 해당 스레드와 다른 스레드 간에 교착 상태가 발생한 대기 중인 작업입니다.<br /><br /> 위로 마우스를 가져가고는 **상태** 셀에는 교착 상태의 작업 또는 대기 작업이 블록에 대 한 추가 정보를 봅니다. **경고:** 는 **작업** 창에서 체인 순회에서는 WCT (Wait) 지원 되는 동기화 기본 형식을 사용 하는 차단 된 작업에 대해서만 교착 상태가 보고 합니다. 예:에서 교착 상태가 발생 <xref:System.Threading.Tasks.Task> 디버거 보고 WCT를 사용 하는 개체 **대기 중-교착 상태**합니다. 디버거 WCT를 사용 하지 않는, 동시성 런타임에서 관리 되는 교착 상태의 작업에 대 한 보고 **대기**합니다. WCT에 대 한 자세한 내용은 참조 [Wait Chain Traversal](http://msdn.microsoft.com/library/ms681622\(VS.85\).aspx)합니다.|  
...
|**AsyncState**|관리 코드의 경우 작업 상태입니다. 기본적으로 이 열은 숨겨집니다. 이 열을 표시하려면 열 머리글 중 하나에 대한 상황에 맞는 메뉴를 엽니다. 선택 **열**, **AsyncState**합니다.|  
