---
title: "원격 컴퓨터에서 앱을 UWP 및 Windows 8.1 실행 | Microsoft Docs"
ms.custom: 
ms.date: 01/05/2018
ms.reviewer: 
ms.suite: 
...
---
# <a name="run-uwp-and-windows-81-apps-on-a-remote-machine"></a>원격 컴퓨터에서 UWP 및 Windows 8.1 앱을 실행 합니다.  
  
원격 컴퓨터에서 UWP 또는 Windows 8.1 앱을 실행 하려면 Visual Studio 용 원격 도구를 사용 하 여 연결 해야 합니다. 원격 도구를 사용 하 여 실행, 디버깅, 프로 파일링 및 Visual Studio를 실행 하는 다른 컴퓨터에서 한 장치에서 실행 되는 UWP 앱을 테스트할 수 있습니다. 원격 장치에서 실행 되는 Visual Studio 컴퓨터에서 터치, 지리적 위치 및 실제 방향 등의 UWP 앱에 관련 된 기능을 지원 하지 않을 때 특히 효과적일 수 있습니다. 이 항목에서는 원격 세션을 구성하고 시작하는 절차에 대해 설명합니다.
...
Windows 10에서 배포할 수 있습니다는 USB로 연결 된 장치를 선택 하 여 **장치** 대신 **원격 컴퓨터** 배포 대상으로 (에서 수행할 수 있습니다는 **표준** 도구 모음 또는 디버깅 속성 페이지에서). Windows 8.1 대 한 직접 연결 하려면 먼저 장치에 원격 도구를 설치 합니다.
...
Windows 8.1 또는 Windows 10의 사전-작성자의 업데이트 버전을 사용 하는 경우 이러한 지침을 따릅니다. 그렇지 않으면이 섹션을 건너뛸 수 있습니다.
