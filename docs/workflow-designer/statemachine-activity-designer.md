...
## <a name="using-the-statemachine-activity-designer"></a>StateMachine 활동 디자이너 사용  
 추가 하는 <xref:System.Activities.Statements.StateMachine> 활동을 끌어는 **StateMachine** 활동 디자이너를는 **상태 시스템** 섹션은 **도구 상자** 에 놓습니다는 [!INCLUDE[wfd1](../workflow-designer/includes/wfd1_md.md)] 화면입니다. 이 자식 상태를 추가 하려면 <xref:System.Activities.Statements.StateMachine> 활동을 끌어는 <xref:System.Activities.Statements.State> 또는 <xref:System.Activities.Core.Presentation.FinalState> 에서 **도구 상자** 놓습니다는 **StateMachine**합니다.  
