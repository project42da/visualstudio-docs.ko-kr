...
### <a name="configuring-a-build"></a>빌드 구성  
 ...
 ![Visual Studio 디버그 빌드 단추](~/docs/ide/media/vs_ide_gs_debug_build_type1.PNG "Vs_ide_gs_debug_build_type1")  
 ...
 **x86**(32비트 Intel CPU), **x64**(64비트 Intel CPU) 및 **ARM**(ARM CPU, 특정 앱 형식에서만 지원됨)과 같은 특정 빌드 플랫폼을 대상으로 지정할 수도 있습니다. 관리되는 프로젝트 및 네이티브 프로젝트에 대한 기본값은 **x86**입니다. 변경하려면 빌드 플랫폼 드롭다운을 클릭하고 다른 플랫폼이나 **구성 관리자...**를 선택합니다.  
 ...
 ![Visual Studio 구성 파일 관리자 창](~/docs/ide/media/vs_ide_gs_debug_build_cf_mgr.PNG "Vs_ide_gs_debug_build_cf_mgr")  

 **구성 관리자**를 사용하여 대상 빌드 구성을 지정할 수 있습니다. 실행하고 **구성** 또는 **CPU** 드롭다운을 클릭한 다음 **새로 만들기...**를 선택하여 새 빌드 또는 플랫폼을 만듭니다.  

 ![Visual Studio 구성 관리자 창](~/docs/ide/media/vs_ide_gs_debug_build_cf_mgr_2.PNG "Vs_ide_gs_debug_build_cf_mgr_2")  
...
### <a name="building-your-code"></a>코드 빌드  
 ...
 ![Visual Studio 빌드 프로젝트 메뉴 선택](~/docs/ide/media/vs_ide_gs_debug_build_menu_item.png "Vs_ide_gs_debug_build_menu_item")  
 ...
 ![Visual Studio 성공적인 빌드 출력](~/docs/ide/media/vs_ide_gs_debug_success_build.PNG "vs_ide_gs_debug_success_build")  
...
### <a name="reviewing-the-error-list"></a>오류 목록 검토  
 ...
 ![Visual Studio 출력 및 오류 목록](~/docs/ide/media/vs_ide_gs_debug_bad_build_error_list.PNG "Vs_ide_gs_debug_bad_build_error_list")  
 ...
 **오류 목록** 창에서 오류 코드 줄을 클릭하여 오류가 발생한 줄로 이동합니다. 또는 오른쪽 위에서 **빠른 실행** 표시줄을 클릭하고 “줄 번호”를 입력한 다음 Enter 키를 눌러 줄 번호를 설정합니다. 이는 줄 번호를 설정할 수 있는 **옵션** 창 항목에 액세스하는 가장 빠른 방법입니다. **빠른 실행** 표시줄을 사용하여 UI 클릭 수를 줄이는 방법을 알아봅니다.  
 ...
 ![줄 번호가 있는 Visual Studio 편집기](~/docs/ide/media/vs_ide_gs_debug_line_numbers.png "Vs_ide_gs_debug_line_numbers")  

 ![Visual Studio 줄 번호 옵션](~/docs/ide/media/vs_ide_gs_debug_options_line_numbers.png "Vs_ide_gs_debug_options_line_numbers")  
 ...
 ![Visual Studio 오류 가리키기](~/docs/ide/media/vs_ide_gs_debug_error_hover1.png "Vs_ide_gs_debug_error_hover1")  

 오류 목록을 통해 코드의 모든 오류를 해결합니다.  

 ![Visual Studio 디버그 오류 창](~/docs/ide/media/vs_ide_gs_debug_error_list.PNG "Vs_ide_gs_debug_error_list")  
...
### <a name="reviewing-errors-in-detail"></a>오류 세부 정보 검토  
 ...
 ![Visual Studio 오류 목록 Bing 검색](~/docs/ide/media/vs_ide_gs_debug_error_list_error_help.png "Vs_ide_gs_debug_error_list_error_help")  
...
### <a name="performing-static-code-analysis"></a>정적 코드 분석 수행  
 ...
 ![Visual Studio 코드 분석 메뉴 항목](~/docs/ide/media/vs_ide_gs_debug_run_code_analysis.png "Vs_ide_gs_debug_run_code_analysis")  

 IDE 아래쪽에 있는 **오류 목록** 탭에 새로운 경고나 업데이트된 경고가 표시됩니다. 경고를 클릭하여 이동합니다.  

 ![Visual Studio 오류 목록(경고 포함)](~/docs/ide/media/vs_ide_gs_debug_code_analysis_warning_error_list.PNG "vs_ide_gs_debug_code_analysis_warning_error_list")  

 경고는 빨간색이 아니라 밝은 노랑-녹색 오류 표시선으로 식별됩니다. 경고 위로 마우스를 가져가서 자세한 내용을 보고, 마우스 오른쪽 단추로 클릭하여 수정 작업이나 리팩터링 옵션을 지원하는 상황에 맞는 메뉴를 표시합니다.  

 ![Visual Studio 코드 분석 경고 가리키기](~/docs/ide/media/vs_ide_gs_debug_code_analysis_warning_hover.png "vs_ide_gs_debug_code_analysis_warning_hover")  
...
### <a name="using-light-bulbs-to-fix-or-refactor-code"></a>전구를 사용하여 코드 수정 또는 리팩터링  
 ...
 ![Visual Studio 전구 빠른 옵션](~/docs/ide/media/vs_ide_gs_debug_light_bulb1.png "Vs_ide_gs_debug_light_bulb1")  

 해당 코드 줄에 적용할 수 있는 가능한 수정 또는 리팩터링 목록이 표시됩니다.  

 ![Visual Studio 전구 미리 보기](~/docs/ide/media/vs_ide_gs_debug_light_bulb_preview_changes.PNG "Vs_ide_gs_debug_light_bulb_preview_changes")  
 ...
 ![Visual Studio 전구 '옵션 없음' 텍스트](~/docs/ide/media/vs_ide_gs_debug_light_bulb_no_options.PNG "Vs_ide_gs_debug_light_bulb_no_options")  
...
### <a name="debugging-your-running-code"></a>실행 중인 코드 디버그  
 성공적으로 코드를 빌드하고 약간의 정리 작업을 수행했으므로 이제 F5 키를 누르거나 **디버그->디버깅 시작**을 선택하여 실행합니다. 그러면 디버그 환경에서 앱이 시작되므로 해당 동작을 자세히 관찰할 수 있습니다. Visual Studio IDE는 앱이 실행되는 동안 변경됩니다. 기본 창 구성에서는 **출력** 창이 **자동/지역/모듈/조사식** 탭 창과 **호출 스택/중단점/예외 설정/출력** 탭 창의 새로운 두 창으로 대체됩니다. 이러한 창에는 실행 시 앱의 변수, 스레드, 호출 스택 및 기타 다양한 동작을 검사 및 평가할 수 있게 해주는 여러 탭이 있습니다.  

 ![Visual Studio 자동 및 호출 스택 창](~/docs/ide/media/vs_ide_gs_debug_autos_and_call_stack.PNG "Vs_ide_gs_debug_autos_and_call_stack")  

 앱에서 다양한 작업을 시도하고 변경 내용을 관찰합니다. 비정상적인 동작이 나타나면 Ctrl+Alt+Break를 누르거나 **일시 중지** 단추를 클릭하여 앱을 일시 중지합니다.  

 ![Visual Studio 모두 중단 단추](~/docs/ide/media/vs_ide_gs_debug_break_all_button.png "vs_ide_gs_debug_break_all_button")  

 F5 키를 누르거나 **계속** 단추를 클릭하여 앱을 계속 실행합니다.  

 ![Visual Studio 디버그 계속 단추](~/docs/ide/media/vs_ide_gs_debug_continue_button.png "Vs_ide_gs_debug_continue_button")  
...
### <a name="setting-simple-breakpoints"></a>간단한 중단점 설정  
 ...
 ![Visual Studio 중단점](~/docs/ide/media/vs_ide_gs_debug_breakpoint1.png "Vs_ide_gs_debug_breakpoint1")  
...
### <a name="setting-conditional-breakpoints"></a>조건부 중단점 설정  
 ...
 ![Visual Studio 중단점 설정](~/docs/ide/media/vs_ide_gs_debug_breakpoint_settings.png "Vs_ide_gs_debug_breakpoint_settings")  

 중단점 발생에 대한 특정 조건을 설정할 수 있는 다음과 같은 대화 상자가 나타납니다.  

 ![Visual Studio 조건부 중단점](~/docs/ide/media/vs_ide_gs_debug_breakpoint_conditional.PNG "Vs_ide_gs_debug_breakpoint_conditional")  

 조건부 중단점 평가에 사용되는 식을 선언하는 방법에 대한 자세한 내용은 Channel9 동영상 [Visual Studio의 중단점 구성 환경](http://channel9.msdn.com/Events/Visual-Studio/Connect-event-2014/711)을 참조하세요.  
...
### <a name="inspecting-your-code-at-run-time"></a>런타임에 코드 검사  
 ...
 ![Visual Studio 런타임 값 검사](~/docs/ide/media/vs_ide_gs_debug_inspect_value.PNG "vs_ide_gs_debug_inspect_value")  
 ...
 또한 Visual Studio에서는 앱의 시간별 CPU 및 메모리 사용량을 관찰할 수 있는 진단 도구 창을 표시합니다. 이 창에서 예기치 않은 높은 CPU 사용량 또는 메모리 할당을 찾습니다. **조사식** 창 및 중단점과 함께 사용하여 예기치 않은 높은 사용량이나 해제되지 않은 리소스의 원인을 확인합니다.  

 ![Visual Studio 진단 도구 창](~/docs/ide/media/vs_ide_gs_debug_diagnostic_tools.PNG "Vs_ide_gs_debug_diagnostic_tools")  

### <a name="running-unit-tests"></a>단위 테스트 실행  
 유닛 테스트는 앱이나 서비스에서 코드 경로를 실행하는 프로그램입니다. Visual Studio에서는 관리 코드 및 네이티브 코드에 대한 Microsoft 유닛 테스트 프레임워크를 설치합니다. 유닛 테스트 프레임워크를 사용하여 유닛 테스트를 만들고, 실행하고, 이러한 테스트 결과를 보고합니다. 변경 시 단위 테스트를 다시 실행하여 코드가 여전히 제대로 작동하는지 테스트합니다. Visual Studio Enterprise Edition을 사용하는 경우 빌드할 때마다 테스트를 자동으로 실행할 수 있습니다.  
