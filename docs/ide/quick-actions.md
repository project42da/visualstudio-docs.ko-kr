...
1. 줄의 임의 위치에서 **Ctrl** + **.**를 눌러 전구를 호출하고 잠재적 해결 방법 목록으로 바로 이동합니다.
...
### <a name="replace-method-with-property-or-replace-property-with-method"></a>메서드를 속성으로 대체 또는 속성을 메서드로 대체
