---
title: "필드, 속성 또는 로컬 생성 - 코드 생성(Visual Basic) | Microsoft Docs"
ms.custom: 
ms.date: 11/17/2016
ms.reviewer: 
ms.suite: 
...
---
# <a name="generate-a-field-property-or-local-in-visual-basic"></a>Visual Basic에서 필드, 속성 또는 로컬 생성
...
1. 존재하지 않는 필드, 로컬 또는 속성을 사용했음을 나타내는 빨간색 구부러진 곡선이 있는 줄에 커서를 놓습니다.
   ...
   ![강조 표시된 코드](media/field-highlight-vb.png)

1. 다음 작업 중 하나를 수행합니다.
   * **키보드**
     * **Ctrl+.**를 눌러 **빠른 작업 및 리팩터링** 메뉴를 트리거하고 [미리 보기] 창 팝업에서 **‘*Name*’ 변수 생성 > 필드/속성/로컬 생성**을 선택합니다.
   * **마우스**
     * 마우스 오른쪽 단추를 클릭하고, **빠른 작업 및 리팩터링** 메뉴를 선택하고, [미리 보기] 창 팝업에서 **‘*Name*’ 변수 생성 > 필드/속성/로컬 생성**을 선택합니다.
   ...
   ![필드/속성/로컬 생성 미리 보기](media/field-preview-vb.png)
...
1. 필드, 속성 또는 로컬은 해당 사용에서 추론된 형식을 사용하여 자동으로 만들어집니다.
   ...
   ![필드/속성/로컬 생성 결과](media/field-result-vb.png)
