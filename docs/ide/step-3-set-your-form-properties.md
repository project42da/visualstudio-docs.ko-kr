...
3.  폼을 선택한 후 **속성** 창에서 **Text** 속성을 찾습니다.  목록을 정렬 방법에 따라 아래로 스크롤해야 할 수도 있습니다.  **Text**를 선택하고 사진 뷰어를 입력한 다음 Enter 키를 선택합니다.  이제 폼의 제목 표시줄에 **사진 뷰어**라는 텍스트가 표시되고 **속성** 창이 다음 그림과 같이 표시됩니다.  
  
     ![속성 창](../ide/media/express_edittextproperty.png "Express\_EditTextProperty")  
속성 창  
...
5.  **속성** 창을 살펴보면 **Size** 속성이 변경되었음을 알 수 있습니다.  **Size** 속성은 폼의 크기를 조정할 때마다 변경됩니다.  폼의 핸들을 끌어 폼의 크기를 이 프로젝트에 적합한 크기인 약 550, 350\(정확할 필요는 없음\)으로 조정합니다.  또는 **Size** 속성에 값을 직접 입력한 다음 Enter 키를 선택할 수도 있습니다.  
...
6.  프로그램을 다시 실행합니다.  프로그램을 실행하려면 다음 방법 중 하나를 사용할 수 있습니다.  
    ...
    -   도구 모음에서 **디버깅 시작** 단추를 선택하면 다음과 같이 표시됩니다.  
         ...
         ![디버깅 도구 모음 시작 단추](../ide/media/express_icondebug.png "Express\_IconDebug")  
디버깅 시작 도구 모음 단추  
...
-   다음 자습서 단계로 이동하려면 [4단계: TableLayoutPanel 컨트롤을 사용하여 폼 레이아웃](../ide/step-4-lay-out-your-form-with-a-tablelayoutpanel-control.md)을 참조하십시오.  
  
-   이전 자습서 단계로 돌아가려면 [2단계: 프로그램 실행](../ide/step-2-run-your-program.md)을 참조하십시오.