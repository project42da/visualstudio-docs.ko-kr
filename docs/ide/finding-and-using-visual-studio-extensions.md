...
## <a name="extension-crash-notifications"></a>확장 작동 중단 알림

Visual Studio 2017(버전 15.3 - 미리 보기)에서 Visual Studio는 이전 세션 중에 확장이 작동 중단에 관련된 것으로 의심되는 경우 사용자에게 알립니다. Visual Studio의 작동이 중단되면 Visual Studio에서는 예외 스택을 저장합니다. 다음번에 Visual Studio가 시작되면 Visual Studio에서는 리프로 시작하고 베이스로 진행하여 스택을 검사합니다. Visual Studio에서는 프레임이 설치되고 사용하도록 설정된 확장의 일부인 모듈에 속한다고 판단할 경우 다음과 같은 메시지로 알립니다.

  “이전 세션이 예기치 않게 종료되었습니다. ‘extension_name’ 확장을 사용하지 않도록 설정하면 유사한 문제를 방지할 수 있습니다.”

알림을 무시하거나 다음 작업 중 하나를 수행할 수 있습니다.
...
-   **이 확장에 대해 다시 표시 안 함**을 선택합니다. IDE에서는 이 확장과 관련된 작동 중단에 대한 알림을 더 이상 표시하지 않지만, 다른 확장과 관련된 작동 중단에 대한 알림을 표시합니다.

-   **자세한 정보**를 선택하여 기본 브라우저에서 도움말 항목을 확인합니다.

-   알림 끝에서 **X** 단추를 선택하여 알림을 해제합니다. 동일한 확장이 이후 세션에서 발생하는 작동 중단에 관련될 경우 알림이 다시 나타납니다.

> [!NOTE]
>  작동 중단 알림은 확장 모듈 중 하나가 작동 중단에 대한 스택에 있었다는 것만 의미합니다. 확장 자체로 인해 작동 중단이 발생했다는 의미가 아닐 수 있습니다. 확장이 Visual Studio에 포함된 코드를 호출하고 해당 코드로 인해 작동 중단이 발생했을 수 있습니다. 그러나 작동 중단을 유발한 시나리오가 중요하지 않을 경우 알림이 유용할 수 있습니다. 이 경우 확장을 사용하지 않도록 설정하면 생산성에 영향을 미치지 않고 이후에 동일한 작동 중단을 피할 수 있습니다.
