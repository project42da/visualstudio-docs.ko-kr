...
helpviewer_keywords: 
  ...
  - "문서 개요 창"
  - "개체 브라우저"
  - "Visual Studio, 호출 계층 구조 창"
  - "Visual Studio, 클래스 뷰"
  - "Visual Studio, 코드 정의 창"
  - "Visual Studio, 문서 개요 창"
  - "Visual Studio, 개체 브라우저"
...
Visual Studio 프로젝트의 개체 및 멤버, .NET Framework 구성 요소의 개체 및 멤버, COM 구성 요소, DLL\(동적 연결 라이브러리\) 및 TLB\(형식 라이브러리\)를 검사할 수 있습니다.  
 ...
 Visual Studio Enterprise가 있는 경우 코드 맵을 사용하여 코드의 구조와 전체 솔루션에서의 해당 종속성을 시각화하고 관심이 있는 코드 부분으로 드릴다운할 수 있습니다.  자세한 내용은 [솔루션 전체의 종속성 매핑](../modeling/map-dependencies-across-your-solutions.md)을 참조하세요.  
...
|**사전순 정렬**|이름을 기준으로 사전순으로 형식 및 멤버를 나열합니다.|  
|**개체 유형별 정렬**|유형을 기준으로 순서대로 형식 및 멤버를 나열합니다\(예: 클래스가 인터페이스보다 선행하고, 인터페이스가 대리자보다 선행하고, 메서드가 속성보다 선행\).|  
|**개체 액세스별 정렬**|공용 또는 전용과 같이 액세스 유형을 기준으로 순서대로 형식 및 멤버를 나열합니다.|  
|**개체 유형별 그룹화**|개체 유형별 그룹으로 형식 및 멤버를 정렬합니다.|  
...
### 문서 개요 창  
 XAML 페이지용 디자이너 또는 Windows Form 디자이너와 같은 디자이너 보기와 함께 또는 HTML 페이지와 함께 **문서 개요** 창을 사용할 수 있습니다.  이 창은 폼 또는 페이지의 논리 구조를 확인하고 깊이 포함되거나 숨겨진 컨트롤을 찾을 수 있도록 트리 뷰에 요소를 표시합니다.  
