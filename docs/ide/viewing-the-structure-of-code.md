...
창 맨 위에 있는 드롭다운 목록을 사용하여 보려는 구성 요소를 필터링할 수 있습니다. 사용자 지정 구성 요소에는 관리 코드 실행 파일, 라이브러리 어셈블리, 형식 라이브러리 및 .ocx 파일이 포함될 수 있습니다. C++ 사용자 지정 구성 요소는 추가할 수 없습니다. 사용자 지정 설정은 Visual Studio 사용자 응용 프로그램 디렉터리, %APPDATA%\Microsoft\VisualStudio\15.0\ObjBrowEX.dat에 저장됩니다.
