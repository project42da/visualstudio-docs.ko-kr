...
### <a name="dependentos"></a>dependentOS  
 선택적 요소. 포함 된 `osVersionInfo` 요소입니다. `dependentOS` 및 `dependentAssembly` 요소는 함께: 둘 중 하나에 대 한 있어야는 `dependency` 요소 중 하나만 있습니다.  
  
 `dependentOS` 다음과 같은 특성을 지원합니다.  
  
|특성|설명|  
|---------------|-----------------|  
|`supportUrl`|선택적 요소. 종속 플랫폼에 대 한 지원 URL을 지정합니다. 필요한 플랫폼을 찾은 경우이 URL은 사용자에 게 표시 됩니다.|  
|`description`|선택적 요소. 에 설명 된 운영 체제를 이해 하기 쉬운 형태로 설명는 `dependentOS` 요소입니다.|  
...
|`servicePackMinor`|선택적 요소. 운영 체제 서비스 팩 부 버전 번호를 지정합니다.|  
|`productType`|선택적 요소. 제품 유형 값을 식별합니다. 유효한 값은 `server`, `workstation` 및 `domainController`입니다. 예를 들어, Windows 2000 Professional이 특성 값은 `workstation`합니다.|  
|`suiteType`|선택적 요소. 시스템 또는 시스템의 구성 유형을 사용할 수 있는 제품군을 식별합니다. 유효한 값은 `backoffice`, `blade`, `datacenter`, `enterprise`, `home`, `professional`, `smallbusiness`, `smallbusinessRestricted`, 및 `terminal`합니다. 예를 들어, Windows 2000 Professional이 특성 값은 `professional`합니다.|  
  
### <a name="dependentassembly"></a>dependentAssembly  
 선택적 요소. 포함 된 `assemblyIdentity` 요소입니다. `dependentOS` 및 `dependentAssembly` 요소는 함께: 둘 중 하나에 대 한 있어야는 `dependency` 요소 중 하나만 있습니다.  
...
|`publicKeyToken`|선택적 요소. 마지막 8 바이트를 나타내는 16 자 16 진수 문자열을 지정 된 `SHA-1` 응용 프로그램이 나 어셈블리 서명에 사용 된 공개 키의 해시 값입니다. 카탈로그 서명에 사용 되는 공개 키 2048 비트 이상 이어야 합니다.|  
|`processorArchitecture`|선택적 요소. 프로세서를 지정합니다. 유효한 값은 `x86` 32 비트 Windows에 대 한 및 `I64` 64 비트 Windows에 대 한 합니다.|  
|`language`|선택적 요소. 두 부분 언어 코드 등 EN-US, 어셈블리를 식별합니다.|  
