...
|특성|설명|  
|---------------|-----------------|  
|`preRequisite`|선택적 요소. 이 어셈블리는 GAC에 이미 존재 해야 지정 합니다. 유효한 값은 `true` 및 `false`입니다. 경우 `true`, 지정된 된 어셈블리는 GAC에 존재 하지 않는 하 고, 응용 프로그램이 실행 되지 않습니다.|  
|`visible`|선택적 요소. 종속성을 비롯 한 최상위 응용 프로그램 id를 식별 합니다. 내부적으로 사용 [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] 응용 프로그램 저장소 및 정품 인증을 관리할 수 있습니다.|  
|`dependencyType`|필수 요소. 이 종속성 및 응용 프로그램 간의 관계입니다. 올바른 값은 다음과 같습니다.<br /><br /> -   `install`. 구성 요소는 현재 응용 프로그램에서 별도 설치를 나타냅니다.<br />-   `preRequisite`. 현재 응용 프로그램 구성 요소가 필요 합니다.|  
|`codebase`|선택적 요소. 응용 프로그램 매니페스트를 전체 경로입니다.|  
|`size`|선택적 요소. 응용 프로그램 매니페스트의 바이트 크기입니다.|  
  
## <a name="assemblyidentity"></a>assemblyIdentity  
 필수 요소. 이 요소는 `dependentAssembly` 요소의 자식입니다. 내용을 `assemblyIdentity` 에 설명 된 대로 동일 해야 합니다는 [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] 응용 프로그램 매니페스트 합니다. 다음 표에의 특성은 `assemblyIdentity` 요소입니다.  
  
|특성|설명|  
|---------------|-----------------|  
|`Name`|필수 요소. 응용 프로그램의 이름을 식별합니다.|  
|`Version`|필수 요소. 다음 형식으로는 응용 프로그램의 버전 번호를 지정합니다. `major.minor.build.revision`|  
|`publicKeyToken`|필수 요소. 응용 프로그램이 나 어셈블리 서명에 사용 된 공개 키의 sha-1 해시의 마지막 8 바이트를 나타내는 16 자리의 16 진수 문자열을 지정 합니다. 서명에 사용 되는 공개 키에 2048 비트 이어야 합니다. 큰 합니다.|  
|`processorArchitecture`|필수 요소. 마이크로프로세서를 지정합니다. 유효한 값은 `x86` 32 비트 Windows에 대 한 및 `IA64` 64 비트 Windows에 대 한 합니다.|  
|`Language`|선택적 요소. 어셈블리의 두 부분 언어 코드를 식별합니다. 예를 들어, EN-US, 영어 (미국)를 나타내는입니다. 기본값은 `neutral`입니다. 이 요소는는 `asmv2` 네임 스페이스입니다.|  
|`type`|선택적 요소. 이전 버전인 Windows-함께와 호환성 설치 기술 합니다. 허용 되는 유일한 값은 `win32`합니다.|  
