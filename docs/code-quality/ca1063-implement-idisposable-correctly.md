...
-   봉인 되지 않은 형식에 대 한 Finalize 구현 중 하나 또는 모두가 Dispose(bool) 또는 사례 클래스 종료자를 호출 하지 않습니다.  
 ...
 봉인 되지 않은 모든 루트 IDisposable 형식 자체 보호 된 가상 void Dispose(bool) 메서드를 제공 해야 합니다. Dispose () Dipose(true)를 호출 해야 하 고 Finalize와 호출 해야 합니다. 봉인 되지 않은 루트 IDisposable 종류를 만드는 경우 Dispose(bool)를 정의 하 고 메서드를 호출 해야 합니다. 자세한 내용은 참조 [관리 되지 않는 리소스 정리](/dotnet/standard/garbage-collection/unmanaged) 에 [Framework 디자인 지침](/dotnet/standard/design-guidelines/index) .NET Framework 설명서의 섹션입니다.  
...
-   봉인 되지 않은 루트 IDisposable 클래스를 작성 하는 경우 IDisposable 구현이 단원의 앞에서 설명한 패턴을 따르는지를 확인 합니다.  
