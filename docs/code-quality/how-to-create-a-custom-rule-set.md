...
Visual Studio에서 있습니다 만들고 수정할 수는 사용자 지정 *규칙 집합* 코드 분석와 관련 된 특정 프로젝트 요구 사항에 맞게 합니다. 사용자 지정 규칙 집합을 만들려면, 또는 하나를 더 표준 규칙 규칙 집합 편집기에 설정 합니다. 다음 추가 하거나 특정 규칙을 제거할 수 있으며 코드 분석 규칙 위반을 판단 하는 경우 발생 하는 동작을 변경할 수 있습니다.
