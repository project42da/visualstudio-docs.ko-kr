---
title: "연습: 코드 오류에 대 한 관리 되는 코드 분석 | Microsoft Docs"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
이 연습에서는 코드 분석 도구를 사용 하 여 코드 오류에 대 한 관리 되는 프로젝트를 분석 합니다.  
 ...
 이 연습에서는 Microsoft.NET Framework 디자인 지침 준수에 대 한.NET 관리 되는 코드 어셈블리를 분석 하려면 코드 분석을 사용 하는 과정입니다.  
...
4.  다음 사항을 확인 **빌드에 코드 분석 사용 (CODE_ANALYSIS 상수 정의**)을 선택 합니다.  
  
5.  **이 규칙 집합 실행** 드롭 다운 목록 **Microsoft 모든 규칙**합니다.  
  
6.  에 **파일** 메뉴를 클릭 하 여 **선택한 항목 저장**, ManagedDemo 속성 페이지를 닫습니다.  
  
7.  에 **빌드** 메뉴를 클릭 하 여 **ManagedDemo 빌드**합니다.  
  
     CodeAnalysisManagedDemo 프로젝트 빌드 경고에는 보고는 **코드 분석** 및 **출력** windows 합니다.  
  
     경우는 **코드 분석** 창이 표시 되지 않으면에 **분석** 메뉴 선택 **Windows** 차례로 **코드 분석**합니다.  
...
1.  에 **보기** 메뉴를 클릭 하 여 **오류 목록**합니다.  
     ...
     선택한 개발자 프로필에 따라를 가리키도록 할 수 있습니다 **다른 창** 에 **보기** 메뉴를 차례로 클릭 **오류 목록**합니다.  
  
2.  **솔루션 탐색기**, 클릭 **모든 파일 표시**합니다.  
  
3.  이제 속성 노드를 확장 한 다음 AssemblyInfo.cs 파일을 엽니다.  
...
-   [CA1014: CLSCompliantAttribute로 어셈블리 표시](../code-quality/ca1014-mark-assemblies-with-clscompliantattribute.md): Microsoft.Design: '데모'는 CLSCompliantAttribute로 표시 해야 하 고 해당 값은 true 여야 합니다.  
    ...
    -   코드를 추가 `using``System;` AssemblyInfo.cs 파일에 있습니다.  
         ...
         다음으로 코드를 추가 `[assembly: CLSCompliant(true)]` AssemblyInfo.cs 파일의 끝입니다.  
...
-   [CA1032: 표준 예외 생성자를 구현](../code-quality/ca1032-implement-standard-exception-constructors.md): Microsoft.Design:이 클래스에 다음 생성자를 추가 합니다: 공용 demo(String)  
    ...
    -   생성자를 추가 `public demo (String s) : base(s) { }` 클래스 `demo`합니다.  
...
-   [CA1032: 표준 예외 생성자를 구현](../code-quality/ca1032-implement-standard-exception-constructors.md): Microsoft.Design:이 클래스에 다음 생성자를 추가: 공개 데모 (String, 예외)  
    ...
    -   생성자를 추가 `public demo (String s, Exception e) : base(s, e) { }` 클래스 `demo`합니다.  
...
-   [CA1032: 표준 예외 생성자를 구현](../code-quality/ca1032-implement-standard-exception-constructors.md): Microsoft.Design:이 클래스에 다음 생성자를 추가 합니다: 공용 demo()  
    ...
    -   생성자를 추가 `public demo () : base() { }` 클래스 `demo` **합니다.**  
...
-   [CA2210: 어셈블리 이름을 사용 해야 올바른 강력한](../code-quality/ca2210-assemblies-should-have-valid-strong-names.md): 'ManagedDemo' 강력한 이름 키로 서명 합니다.  
...
-   [CA2237: ISerializable 형식을 SerializableAttribute로 표시](../code-quality/ca2237-mark-iserializable-types-with-serializableattribute.md): Microsoft.Usage:이 형식이 ISerializable을 구현 하 '는 데모' 형식으로 [Serializable] 특성을 추가 합니다.  
    ...
    -   추가 `[Serializable ()]` 특성을 클래스 `demo`합니다.  
...
1.  나머지 경고 각각에 대해 다음을 수행합니다.  
    ...
    2.  선택 **작업**를 눌러 **메시지 표시 안 함**, 선택한 후 **프로젝트 억제 (suppression) 파일에서**합니다.  
