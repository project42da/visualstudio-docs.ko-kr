...
이 항목에서는 [!INCLUDE[vsPreShort](../code-quality/includes/vspreshort_md.md)] 프로파일링 도구 명령줄 도구를 사용하여 [!INCLUDE[dnprdnshort](../code-quality/includes/dnprdnshort_md.md)] 서비스를 계측하고 자세한 타이밍 데이터를 수집하는 방법을 설명합니다.  
...
2.  **VSInstr** 도구를 사용하여 서비스 이진의 계측된 버전을 생성합니다.  
...
7.  프로파일러를 시작합니다. 형식:  
    ...
    |[/waitstart](../profiling/waitstart.md)[**:**`Interval`]|프로파일러가 오류를 반환하기 전에 초기화될 때까지 기다리는 시간(초)을 지정합니다. `Interval`이 지정되지 않은 경우 프로파일러는 무기한 기다립니다. 기본적으로 **/start**는 즉시 반환합니다.|  
...
## <a name="controlling-data-collection"></a>데이터 수집 제어  
 서비스가 실행 중이면 **VSPerfCmd.exe** 옵션을 사용하여 프로파일러 데이터 파일에 대한 데이터 쓰기를 시작하고 중지할 수 있습니다. 데이터 수집을 제어하면 서비스의 시작 또는 종료와 같이 프로그램 실행의 특정 부분에 대한 데이터를 수집할 수 있습니다.  
...
## <a name="ending-the-profiling-session"></a>프로파일링 세션 종료  
 ...
 적용될 새 환경 설정에 대한 컴퓨터를 다시 시작해야 합니다.  
...
3.  모든 프로파일링을 완료하면 프로파일링 환경 변수를 지웁니다. 유형:  
  
     **VSPerfClrEnv /globaloff**  
  
4.  계측된 모듈을 원래로 바꿉니다. 필요한 경우 서비스의 시작 유형을 다시 구성합니다.  
