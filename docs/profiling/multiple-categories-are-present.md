---
title: "범주가 여러 개 있는 경우 | Microsoft Docs"
ms.custom: ""
ms.date: "12/03/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "vs-ide-debug"
ms.tgt_pltfrm: ""
ms.topic: "article"
f1_keywords: 
  - "vs.cv.threads.timeline.multicategory"
helpviewer_keywords: 
  - "동시성 시각화 도우미, 범주가 여러 개 있는 경우"
ms.assetid: f5c7ec07-8052-4df7-a0ca-7ec8bb3eee18
caps.latest.revision: 7
caps.handback.revision: 7
author: "mikejo5000"
ms.author: "mikejo"
manager: "ghogen"
---
# 범주가 여러 개 있는 경우
[!INCLUDE[vs2017banner](../code-quality/includes/vs2017banner.md)]

특정 시간 범위 동안 둘 이상의 작업 유형이 포함된 구역이 선택되었습니다.  이때 두 가지 색이 세로로 나란히 표시됩니다.  이러한 색은 특정 시간 범위 동안 가장 빈도가 높은 상위 두 개의 작업만을 나타냅니다.  이 두 작업의 빈도는 상대적으로 세로 막대의 1\/3 또는 1\/2를 차지하는 것으로 표시됩니다.  보다 자세히 보려면 스레드 뷰의 이 구역을 확대해 봅니다.  
  
## 참고 항목  
 [실행 시간\(스레드 뷰\)](../profiling/execution-time-threads-view.md)   
 [I\/O 시간\(스레드 뷰\)](../profiling/i-o-time-threads-view.md)   
 [메모리 관리 시간](../profiling/memory-management-time.md)   
 [선점 시간](../profiling/preemption-time.md)   
 [중지 시간](../profiling/sleep-time.md)   
 [동기화 시간](../profiling/synchronization-time.md)   
 [UI 처리 시간](../profiling/ui-processing-time.md)