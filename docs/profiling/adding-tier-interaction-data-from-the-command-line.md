 ...
 **원격 컴퓨터에서 TIP 데이터 수집**  
 ...
 원격 컴퓨터에서 계층 상호 작용 데이터를 수집하려면 Visual Studio 컴퓨터의 **vs_profiler_***\<Platform>***_***\<Language>***.exe** file from the *%VSInstallDir%***\Team Tools\Performance Tools\Setups** 폴더를 원격 컴퓨터로 복사하여 설치해야 합니다. [원격 디버깅](../debugger/remote-debugging.md) 다운로드 패키지의 프로파일링 도구를 사용할 수 없습니다.  
  
 **TIP 보고서**  
  
 계층 상호 작용 데이터는 [!INCLUDE[vsUltLong](../code-quality/includes/vsultlong_md.md)] IDE에서만 볼 수 있습니다. [VSPerfReport](../profiling/vsperfreport.md)를 통한 파일 기반 계층 상호 작용 보고서를 사용할 수 없습니다.  
