...
계측 프로파일링 방법을 사용하여 프로파일링 데이터 수집을 응용 프로그램의 DLL 하나 이상으로 제한할 수 있습니다. 응용 프로그램에서 하나 이상의 DLL을 프로파일링하려면 .dll 파일이 대상으로 포함된 성능 세션을 만듭니다. 프로파일링할 DLL을 [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] 솔루션의 프로젝트로 또는 독립 이진 파일로 지정할 수 있습니다.  
...
7.  **대상 프로젝트 추가** 목록에서 DLL을 실행하는 데 사용할 실행 가능한 프로젝트를 선택합니다.  
     ...
     선택적 요소. 프로파일링하려는 모든 DLL 프로젝트를 추가할 수 있습니다.  
...
4.  마법사의 두 번째 페이지에서 다음 단계를 수행합니다.  
    ...
    -   선택적 요소. **명령줄 인수**로 실행 파일에 전달할 명령줄 인수를 입력합니다. 필요한 경우 **작업 디렉터리**에서 응용 프로그램의 작업 디렉터리를 지정합니다.  
...
7.  선택적 요소. 다른 .dll 파일을 추가하려면 **대상**을 마우스 오른쪽 단추로 클릭하고 **대상 이진 파일 추가**를 선택합니다. **대상 이진 파일 추가** 대화 상자에서 파일을 선택합니다.  
