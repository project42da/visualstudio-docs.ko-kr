---
title: "연습: 내 첫 WPF 데스크톱 응용 프로그램 | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
...
5.  XAML 창에서 **Height** 요소의 값을 `375`로 설정하고 **Width** 속성의 값을 `500`으로 설정합니다.  
...
2.  **새 항목 추가** 대화 상자에서 **이름** 텍스트 상자를 선택하고 `ExpenseItHome`을 입력한 다음 **추가** 단추를 선택합니다.  
...
6.  줄 `Title="ExpenseIt" Height="375" Width="500">` 요소를 찾아서 `Source="ExpenseItHome.xaml"` 속성을 추가합니다.  
    ...
    ```  
     ...
     앞에서 설정한 속성과 마찬가지로 **속성** 창의 **기타** 범주에서 `Source` 속성을 설정했을 수 있습니다.  
  
#### 세부 정보 창을 추가하려면  
  
1.  **솔루션 탐색기**에서 **ExpenseIt** 노드의 바로 가기 메뉴를 열고 **추가**, **페이지**를 선택합니다.  
  
2.  **새 항목 추가** 대화 상자에서 **이름** 텍스트 상자를 선택하고 `ExpenseReportPage`를 입력한 다음 **추가** 단추를 선택합니다.  
...
5.  메뉴 모음에서 **디버그**, **디버깅 시작**을 선택하거나 F5 키를 눌러 응용 프로그램을 실행합니다.  
     ...
     ![ExpenseIt 샘플 스크린 샷](../designers/media/gettingstartedfigure1.png "GettingStartedFigure1")  
...
2.  **속성** 창에서 **레이아웃** 범주 노드를 확장하고 **여백** 값을 왼쪽, 오른쪽, 위쪽 및 아래쪽 여백에 해당하는 `10`, `10`, `0` 및 `10`으로 설정합니다.  
...
3.  응용 프로그램을 빌드 및 실행합니다. 다음 그림은 이 절차에서 XAML로 생성된 컨트롤의 런타임 모양을 보여 줍니다.  
     ...
     ![ExpenseIt 샘플 스크린 샷](../designers/media/gettingstartedfigure2.png "GettingStartedFigure2")  
  
4.  디자인 모드로 돌아가려면 응용 프로그램을 닫습니다.  
  
#### 배경 이미지를 추가하려면  
  
1.  다음 이미지를 선택하고 `watermark.png`로 저장합니다.  
  
     ![Watermark image for walkthrough](../designers/media/wpf_watermark.png "WPF\_watermark")  
  
    > [!NOTE]
    >  또는 사용자 고유의 이미지를 만들고 `watermark.png`로 저장할 수 있습니다.  
...
4.  `Grid.Column` 값을 1로 설정하여 컨트롤을 두 번째 열로 이동합니다. 각 `Grid.Row` 값을 1씩 증가하여 각 컨트롤을 한 행 아래로 이동합니다.  
    ...
    3.  `<Button Grid.Column="0" Grid.Row="2" Margin="0,10,0,0" Width="125"` 줄을 찾습니다.`Grid.Column="0"`을 `Grid.Column="1"`로 변경하고 `Grid.Row="2"`를 `Grid.Row="3"`으로 변경합니다.  
...
6.  이때 응용 프로그램을 빌드 및 실행하는 경우 다음 그림과 같이 표시되어야 합니다.  
     ...
     ![ExpenseIt 샘플 스크린 샷](../designers/media/gettingstartedfigure3.png "GettingStartedFigure3")  
...
4.  **보기** 단추를 선택합니다.  
     ...
     ![ExpenseIt 샘플 스크린 샷](../designers/media/gettingstartedfigure4.png "GettingStartedFigure4")  
...
2.  다음 XAML을 `<Application.Resources>` 및 `</Application.Resources>` 태그 사이에 추가합니다.  
    ...
    -   `buttonStyle`: **ExpenseItHome.xaml** pppage에 있는 `Button`의 형식을 지정합니다.  
...
3.  `<Grid.Resources>` 요소 내에 다음 XAML 코드를 추가합니다.  
    ...
    ```  
     ...
     이렇게 하면 **ListBox**에 데이터를 표시하는 방법을 정의하는 `Data Template`이 추가됩니다.  
...
4.  기존 `<ListBox>` 요소를 다음 XAML로 바꿉니다.  
    ...
    ```  
     ...
     이렇게 하면 `ListBox`의 `ItemsSource` 속성이 데이터 원본에 바인딩되고 데이터 템플릿이 `ItemTemplate`으로 적용됩니다.  
...
#### 데이터 템플릿을 사용하여 UI를 업데이트하려면  
  
1.  **ExpenseReportPage.xaml**을 엽니다.  
  
2.  **Name** 및 **Department** `<StackPanel` 요소에 대한 XAML 코드를 다음으로 바꿉니다.  
