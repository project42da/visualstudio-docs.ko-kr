---
title: "게임 또는 응용 프로그램에 3차원 자산 사용 | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
...
## 프로젝트 구성  
 3\-D 자산을 빌드의 일부로 배포하려면 [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]에서 사용자가 배포하려는 자산의 종류를 알아야 합니다.  [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]에서는 이미 여러 일반 파일 형식을 알고 있지만 특정 종류의 앱에서만 3\-D 자산을 사용하므로 [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]에서는 프로젝트가 이러한 종류의 파일을 빌드할 것이라고 가정하지 않습니다.  [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]에 여러 가지 형식의 파일을 유용한 방식으로 처리하는 방법을 알려주는 파일인 *빌드 사용자 지정*을 사용하여 사용자 앱에서 이러한 종류의 자산을 사용할 것임을 [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]에 알릴 수 있습니다. 빌드 사용자 지정 파일은 각 자산 형식에 대해 제공됩니다.  이러한 사용자 지정은 프로젝트 단위로 적용되므로 프로젝트에 적절한 사용자 지정을 추가하기만 하면 됩니다.  
  
#### 프로젝트에 빌드 사용자 지정을 추가하려면  
  
1.  **솔루션 탐색기**에서 프로젝트의 바로 가기 메뉴를 열고 **빌드 종속성**, **빌드 사용자 지정**을 차례대로 선택합니다.  **Visual C\+\+ 빌드 사용자 지정 파일** 대화 상자가 표시됩니다.  
  
2.  **사용 가능한 빌드 사용자 지정 파일**에서 다음 테이블에서 설명하는 대로 프로젝트에서 사용하려는 자산 형식에 해당하는 확인란을 선택합니다.  
...
1.  **솔루션 탐색기**의 프로젝트에서 자산의 바로 가기 메뉴를 열고 **속성**을 선택합니다.  자산의 **속성 페이지** 대화 상자가 표시됩니다.  
  
2.  **구성** 및 **플랫폼** 속성이 변경 내용을 적용하려는 값으로 설정되어 있는지 확인합니다.  
  
3.  **구성 속성**에서 **일반**을 선택한 다음 속성 표의 **일반**에서 **항목 형식** 속성을 적절한 현재 파이프라인 항목 형식으로 설정합니다.  예를 들어 이미지 또는 질감 파일의 경우 **이미지 콘텐츠 파이프라인**을 선택합니다.  
  
    > [!IMPORTANT]
    >  기본적으로 [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]에서는 [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]로 빌드되는 **이미지** 항목을 사용하여 여러 종류의 이미지 파일을 범주화해야 한다고 가정합니다.  따라서 이미지 콘텐츠 파이프라인으로 처리하려고 하는 각 이미지의 **항목 형식** 속성을 변경해야 합니다.  3\-D 모델과 시각적 셰이더 그래픽의 다른 콘텐츠 파이프라인 원본 파일 형식은 기본적으로 올바른 **항목 형식**으로 지정됩니다.  
...
1.  **솔루션 탐색기**의 프로젝트에서 자산 파일의 바로 가기 메뉴를 열고 **속성**을 선택합니다.  자산의 **속성 페이지** 대화 상자가 표시됩니다.  
  
2.  **구성** 및 **플랫폼** 속성이 변경 내용을 적용하려는 값으로 설정되어 있는지 확인합니다.  
  
3.  **구성 속성**에서 콘텐츠 파이프라인 노드\(예: 질감 및 이미지 자산의 경우**이미지 콘텐츠 파이프라인**\)를 선택한 다음 속성 표에서 속성을 적절한 값으로 설정합니다.  예를 들어 빌드 시 질감 자산의 Mip 맵을 생성하려면 **Mip 생성** 속성을 **예**로 설정합니다.  
...
|속성|설명|  
|--------|--------|  
|**압축**|출력 파일에 사용할 압축 형식을 지정합니다.<br /><br /> 사용 가능한 옵션은 다음과 같습니다.<br /><br /> -   **압축 안 함**<br />-   **BC1\_UNORM 압축**<br />-   **BC1\_UNORM\_SRGB 압축**<br />-   **BC2\_UNORM 압축**<br />-   **BC2\_UNORM\_SRGB 압축**<br />-   **BC3\_UNORM 압축**<br />-   **BC3\_UNORM\_SRGB 압축**<br />-   **BC4\_UNORM 압축**<br />-   **BC4\_SNORM 압축**<br />-   **BC5\_UNORM 압축**<br />-   **BC5\_SNORM 압축**<br />-   **BC6H\_UF16 압축**<br />-   **BC6H\_SF16 압축**<br />-   **BC7\_UNORM 압축**<br />-   **BC7\_UNORM\_SRGB 압축**<br /><br /> 다른 버전의 DirectX에서 지원되는 압축 형식에 대한 자세한 내용은 [DXGI에 대한 프로그래밍 가이드](http://go.microsoft.com/fwlink/p/?LinkId=246265)를 참조하세요.|  
|미리 곱한 알파 형식으로 변환|이미지를 출력 파일에서 미리 곱한 알파 형식으로 변환하려면 **예**를 선택하고 그렇지 않으면 **아니요**를 선택합니다.  출력 파일만 변경되고 원본 이미지는 그대로 남습니다.|  
|**Mip 생성**|빌드 시 전체 Mip 체인을 생성한 다음 출력 파일에 포함하려면 **예**를 선택하고 그렇지 않으면 **아니요**를 선택합니다.  **아니요**를 선택했는데 원본 파일에 Mip 맵 체인이 이미 포함되어 있는 경우 출력 파일에는 Mip 체인이 있게 됩니다. 그렇지 않으면 출력 파일에는 Mip 체인이 없습니다.|  
|**콘텐츠 출력**|출력 파일의 이름을 지정합니다. **Important:**  출력 파일의 확장명을 변경하더라도 파일 형식에는 아무런 영향을 미치지 않습니다.|  
...
### 셰이더 사용  
 Direct3D에서는 셰이더 리소스를 만들어 프로그래밍 가능한 그래픽 파이프라인으로 바인딩하기 위한 기능을 제공합니다.  Direct3D에서 셰이더 리소스를 만들어 파이프라인으로 바인딩하는 방법에 대한 자세한 내용은 [HLSL에 대한 프로그래밍 가이드](http://go.microsoft.com/fwlink/p/?LinkID=261521)를 참조하세요.  
