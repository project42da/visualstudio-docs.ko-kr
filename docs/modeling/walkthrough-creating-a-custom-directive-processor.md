---
title: "연습: 사용자 지정 지시문 프로세서 만들기 | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
*지시문 프로세서* 코드를 추가 하 여 회사는 *생성 된 변환 클래스*합니다. 호출 하는 경우는 *지시문* 에서 *텍스트 템플릿*, 텍스트 템플릿에 작성 하는 코드의 나머지 부분에서 지시문이 제공 하는 기능을 사용할 수 있습니다.  
  
 사용자 고유의 사용자 지정 지시문 프로세서를 작성할 수 있으며, 이 프로세서를 사용하여 텍스트 템플릿을 사용자 지정할 수 있습니다. 중 하나 <xref:Microsoft.VisualStudio.TextTemplating.DirectiveProcessor>나 <xref:Microsoft.VisualStudio.TextTemplating.RequiresProvidesDirectiveProcessor>.</xref:Microsoft.VisualStudio.TextTemplating.RequiresProvidesDirectiveProcessor> </xref:Microsoft.VisualStudio.TextTemplating.DirectiveProcessor> 에서 상속 되는 클래스를 만들면 사용자 지정 지시문 프로세서를 만들려면  
...
## <a name="creating-a-custom-directive-processor"></a>사용자 지정 지시문 프로세서 만들기  
 이 연습에서는 사용자 지정 지시문 프로세서를 만듭니다. XML 파일을 읽어에 저장 하는 사용자 지정 지시문을 추가 <xref:System.Xml.XmlDocument>변수, 속성을 통해 노출 하 고.</xref:System.Xml.XmlDocument> "지시문 프로세서 테스트" 단원에서는 텍스트 템플릿에서 이 속성을 사용하여 XML 파일에 액세스합니다.  
  
 사용자 지정 지시문 호출은 다음과 같습니다.  
  
 `<#@ CoolDirective Processor="CustomDirectiveProcessor" FileName="<Your Path>DocFile.xml" #>`  
  
 사용자 지정 지시문 프로세서는 생성된 변형 클래스에 변수와 속성을 추가합니다. 에서는 <xref:System.CodeDom>엔진이 생성 된 변환 클래스에 추가 하는 코드를 만들기 위한 클래스</xref:System.CodeDom> 를 작성 하는 지시문 <xref:System.CodeDom>코드를 작성 하는 클래스의 Visual C# 또는 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)]에 지정 된 언어에 따라는 `language` 의 매개 변수는 `template` 지시문.</xref:System.CodeDom> 지시문 프로세서의 언어와 지시문 프로세서에 액세스하는 텍스트 템플릿의 언어는 일치하지 않아도 됩니다.  
...
3.  코드를 대체 **Class1** 다음 코드를 사용 합니다. 상속 하는 CustomDirectiveProcessor 클래스를 정의 하는이 코드는 <xref:Microsoft.VisualStudio.TextTemplating.DirectiveProcessor>클래스 및 필요한 메서드를 구현 합니다.</xref:Microsoft.VisualStudio.TextTemplating.DirectiveProcessor>  
...
4.  에 대 한 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 만 엽니다는 **프로젝트** 메뉴를 클릭 하 고 **CustomDP 속성**합니다. 에 **응용 프로그램** 탭 **루트 네임 스페이스**, 기본 값을 삭제 `CustomDP`합니다.  
...
1.  실행 된 `regedit` 명령을 시작 메뉴나 명령줄을 사용 하 여 합니다.  
  
2.  위치로 이동 하 여 **HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\VisualStudio\\\*.0\TextTemplating\DirectiveProcessors**, 한 노드를 클릭 합니다.  
...
## <a name="testing-the-directive-processor"></a>지시문 프로세서 테스트  
 ...
 텍스트 템플릿을 사용 하 여 다음의 <xref:System.Xml.XmlDocument>지시문이 만드는 XML을 탐색 하 고 문서 주석을 인쇄 하는 속성입니다.</xref:System.Xml.XmlDocument>  
  
#### <a name="to-create-an-xml-file-for-use-in-testing-the-directive-processor"></a>지시문 프로세서 테스트에 사용할 XML 파일을 만들려면  
  
1.  라는 텍스트 파일을 만듭니다 `DocFile.xml` 텍스트 편집기 (예: 메모장)를 사용 하 여 합니다.  
...
3.  다음 사항을 확인은 **사용자 지정 도구** TestDP.tt의 속성이 `TextTemplatingFileGenerator`합니다.  
...
1.  **솔루션 탐색기**TestDP.tt를 마우스 오른쪽 단추로 클릭 하 고 클릭 한 다음 **사용자 지정 도구 실행**합니다.  
  
     에 대 한 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 사용자 TestDP.txt에 나타나지 않을 **솔루션 탐색기** 기본적으로 합니다. 프로젝트에 할당 된 모든 파일을 표시 하려면 엽니다는 **프로젝트** 메뉴를 클릭 **모든 파일 표시**합니다.  
  
2.  **솔루션 탐색기**TestDP.txt 노드를 확장 한 다음 편집기에서 열려는 TestDP.txt를 두 번 클릭 합니다.  
...
3.  브라우저에서 출력을 표시 하려면 **솔루션 탐색기**TestDP.htm을 마우스 오른쪽 단추로 클릭 하 고 클릭 **브라우저에서 보기**합니다.  
