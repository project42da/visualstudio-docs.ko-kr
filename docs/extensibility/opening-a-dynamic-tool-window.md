...
2.  DynamicWindowPanePackage.cs 파일에서 DynamicWindowPanePackage 선언을 찾습니다. 추가 <xref:Microsoft.VisualStudio.Shell.ProvideToolWindowAttribute> 및 T:Microsoft.VisualStudio.Shell.ProvideToolWindowVisibilityAttribute 도구 창을 등록 하는 특성입니다.  
    ...
    ```  
     ...
     Visual Studio를 닫았다가 경우 지속 되지 않는 일시적인 윈도우로 DynamicWindowPane 라는 도구 창을 등록 됩니다. DynamicWindowPane 열릴 때마다 <xref:Microsoft.VisualStudio.VSConstants.UICONTEXT.SolutionExists_string> 적용 되며 그렇지 않은 경우 종료 합니다.  
