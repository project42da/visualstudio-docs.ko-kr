...
## <a name="implementing-commands"></a>명령 구현  
 ...
 명령을 명령 서비스에 추가 되 면 일련의 명령 중 하나가 됩니다. 해당 명령에 대해서만 제공 하 고 다른 명령에 다른 모든 경우 체인에 전달 하는 명령에 대 한 상태 알림 및 실행 메서드를 구현 하는 경우 주의 합니다. 는 명령을 전달 하지 않으면 (일반적으로 반환 하 여 <xref:Microsoft.VisualStudio.OLE.Interop.Constants>), Visual Studio 제대로 작동 하지 않을 수 있습니다.  
...
-   메서드를 반환 합니다 두 메서드를 구현할 GUID 인식 않지만 명령의 실제로 구현 하 고 있지 않은 경우 <xref:Microsoft.VisualStudio.OLE.Interop.Constants>합니다.  
  
-   GUID와 명령을 모두에서 인식 하는 두 메서드를 구현할 경우 메서드가 명령 플래그 필드의 모든 명령에 설정 해야 (에 `prgCmds` 매개 변수)는 다음 플래그를 사용 하 여:  
  
    -   <xref:Microsoft.VisualStudio.OLE.Interop.OLECMDF>경우 명령이 지원 됩니다.  
  
    -   <xref:Microsoft.VisualStudio.OLE.Interop.OLECMDF>이 명령은 표시 되지 않아야 하는 경우 합니다.  
  
    -   <xref:Microsoft.VisualStudio.OLE.Interop.OLECMDF>명령에서 전환 하 고 검사를 표시 합니다.  
  
    -   <xref:Microsoft.VisualStudio.OLE.Interop.OLECMDF>경우 명령이 활성화 됩니다.  
  
    -   <xref:Microsoft.VisualStudio.OLE.Interop.OLECMDF>바로 가기 메뉴에 나타나는 경우이 명령은 숨겨야 하는 경우.  
  
    -   <xref:Microsoft.VisualStudio.OLE.Interop.OLECMDF>명령 메뉴 컨트롤러는 고을 사용 하지 않는 하지만 해당 드롭다운 메뉴 목록을 비어 있지 않으며 계속 사용할 수 있습니다. (이 플래그는 거의 사용 합니다.)  
...
-   명령을 사용 하 여.vsct 파일에 정의 된 경우는 `TextChanges` 플래그입니다. 다음 매개 변수를 설정 합니다.  
 ...
 특정 명령 지를 나타내려면 반환 <xref:Microsoft.VisualStudio.VSConstants.S_OK>합니다. 다른 모든 명령에 대해 반환 <xref:Microsoft.VisualStudio.OLE.Interop.Constants>합니다.  
...
## <a name="execution-methods"></a>실행 메서드  
 Execute 메서드를 구현을 유사 상태 쿼리 메서드를 구현 합니다. 먼저, 컨텍스트 되지 않았는지 자동화 기능을 확인 합니다. 그런 다음 테스트 모두 GUID 및 명령 id입니다. 경우 GUID 명령 ID를 인식할 수 없습니다, 반환 또는 <xref:Microsoft.VisualStudio.OLE.Interop.Constants>합니다.  
