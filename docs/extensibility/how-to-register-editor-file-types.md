...
1.  제공 된 <xref:Microsoft.VisualStudio.Shell.ProvideEditorExtensionAttribute> VSPackage의 클래스에서 편집기에 대 한 적절 한 매개 변수를 사용 하 여 클래스입니다.  
    ...
    ```  
     ...
     `projectGuid` 에 정의 된 기타 파일 형식에 대 한 guid <xref:Microsoft.VisualStudio.VSConstants.CLSID_MiscellaneousFilesProject>합니다. 결과 파일은 빌드 프로세스의 일부로 잘못 된 있도록 기타 파일 형식이 제공 됩니다.  
