...
이 시리즈의 첫 번째 연습 [기본 프로젝트 시스템, 1 부](../extensibility/creating-a-basic-project-system-part-1.md), 기본 프로젝트 시스템을 만드는 방법을 보여 줍니다. 이 연습에서는 Visual Studio 템플릿, 속성 페이지 및 기타 기능을 추가 하 여 기본 프로젝트 시스템에 작성 합니다. 이 시작 하기 전에 첫 번째 연습을 완료 해야 합니다.  
  
 이 연습에서는 프로젝트 파일 이름 확장명.myproj 있는 프로젝트 유형을 만드는 방법에 설명 합니다. 이 연습을 완료 하려면이 연습에서는 기존 Visual C\# 프로젝트 시스템에서 차용 때문에 고유 언어를 만들 필요가 없습니다.  
...
-   프로젝트 형식 자식 노드 만들기는 **새 프로젝트** 대화 상자입니다.  
  
-   Visual Studio 서식 파일에서 매개 변수 대체를 사용 하도록 설정 합니다.  
  
-   프로젝트 속성 페이지를 만듭니다.  
  
> [!NOTE]
>  이 연습의 단계에에서는 C\# 프로젝트를 기반으로 합니다. 그러나 파일 이름 확장명, 코드 등 관련 사항을 제외 하 고 Visual Basic 프로젝트에 대 한 동일한 단계를 사용할 수 있습니다.  
  
## Visual Studio 템플릿 만들기  
 [기본 프로젝트 시스템, 1 부](../extensibility/creating-a-basic-project-system-part-1.md) 기본 프로젝트 서식 파일을 만들고 프로젝트 시스템에 추가 하는 방법을 보여 줍니다. 또한 Visual Studio와 함께 사용 하 여이 서식 파일을 등록 하는 방법을 보여 줍니다는 <xref:Microsoft.VisualStudio.Shell.ProvideProjectFactoryAttribute> 특성의 시스템 레지스트리에 있는 \\Templates\\Projects\\SimpleProject\\ 폴더의 전체 경로 기록 합니다.  
  
 기본 프로젝트 서식 파일 대신 Visual Studio 템플릿 \(.vstemplate 파일\)를 통해 제어할 수 있습니다 서식 파일에 표시 되는 방식을 **새 프로젝트** 대화 상자와 템플릿 매개 변수를 대체 방법입니다.  .Vstemplate 파일은 소스 파일 프로젝트 시스템 템플릿을 사용 하 여 프로젝트를 만들 때 전달 되는 방법을 설명 하는 XML 파일입니다. 프로젝트 시스템 자체는.zip 파일의 소스 파일 및.vstemplate 파일을 수집 하 여 빌드 및 Visual Studio로 알려져 있는 위치에.zip 파일을 복사 하 여 배포 합니다. 이 프로세스는이 연습의 뒷부분에서 자세히 설명 합니다.  
  
1.  [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)], 를 따라 만든 SimpleProject 솔루션을 엽니다 [기본 프로젝트 시스템, 1 부](../extensibility/creating-a-basic-project-system-part-1.md)합니다.  
  
2.  SimpleProjectPackage.cs 파일에서 찾기는 ProvideProjectFactory 특성입니다. Null, ".\\\\NullPath"와 네 번째 매개 변수 \(프로젝트 템플릿 폴더 경로\)와 두 번째 매개 변수를 \(프로젝트 이름\)를 다음과 같이 바꿉니다.  
...
5.  에 **속성** 창에서 모든 5 집합과 \\Templates\\Projects\\SimpleProject\\ 폴더에 파일의 **빌드 작업** 를 **ZipProject**합니다.  
  
 ![](../extensibility/media/simpproj2.png "SimpProj2")  
  
 위치와 모양에 SimpleProject 프로젝트 형식의 \< TemplateData \> 섹션 결정은 **새 프로젝트** 다음과 같이 대화 상자:  
  
-   \< 이름 \> 요소 이름을 SimpleProject 응용 프로그램 프로젝트 템플릿을 지정 합니다.  
  
-   \< 설명 \> 요소에 표시 된 설명을 포함는 **새 프로젝트** 대화 상자는 프로젝트 템플릿을 선택 합니다.  
  
-   \< I \> 요소 SimpleProject 프로젝트 형식에 함께 표시 되는 아이콘을 지정 합니다.  
  
-   \< ProjectType \> 요소 이름을 프로젝트 형식에는 **새 프로젝트** 대화 상자입니다. 이 이름은 ProvideProjectFactory 특성의 프로젝트 name 매개 변수를 대체합니다.  
  
    > [!NOTE]
    >  \< ProjectType \> 요소와 일치 해야는 `LanguageVsTemplate` 의 인수는 `ProvideProjectFactory` SimpleProjectPackage.cs 파일의 특성입니다.  
  
 \< TemplateContent \> 섹션에 새 프로젝트를 만들 때 생성 되는 이러한 파일에 설명 합니다.  
...
-   AssemblyInfo.cs  
 ...
 모든 파일을 세 개의 `ReplaceParameters` 매개 변수 대체를 사용 하면 true로 설정 합니다.  Program.cs 파일에 `OpenInEditor` 프로젝트를 만들 때 코드 편집기에서 열은 파일을 true로 설정 합니다.  
...
## 최소.vsct 파일 추가  
 Visual Studio는 새 또는 수정 된 Visual Studio 서식 파일을 인식 하도록 설치 모드에서 실행 되어야 합니다. 설치 모드.vsct 파일에 존재 하도록 해야 합니다. 따라서 최소한의.vsct 파일을 프로젝트에 추가 해야 합니다.  
  
1.  SimpleProject 프로젝트에 SimpleProject.vsct 이라는 XML 파일을 추가 합니다.  
...
3.  설정의 **빌드 작업** 이 파일의 **VSCTCompile**합니다. 이렇게 하려면.csproj 파일에만 아니라는 **속성** 창입니다. 다음 사항을 확인은 **빌드 작업** 로 설정 되어이 파일의 **None** 이 시점에서.  
    ...
    5.  SimpleProject 노드를 저장 하 고 다음에 **솔루션 탐색기** 클릭 **프로젝트 다시 로드**합니다.  
  
## Visual Studio 템플릿 만들기 단계를 검토합니다.  
 VSPackage 프로젝트 빌드 시스템에서 일반적으로 실행 Visual Studio 설치 모드에서.vstemplate 파일 변경 되거나.vstemplate 파일을 포함 하는 프로젝트를 다시 작성 하는 경우. 보통 또는 높은 MSBuild의 자세한 정도 설정 하 여 계속 따라갈 수 있습니다.  
  
1.  **도구** 메뉴에서 **옵션**을 클릭합니다.  
  
2.  확장 된 **프로젝트 및 솔루션** 노드를 선택한 다음 **빌드 및 실행**합니다.  
  
3.  설정 **MSBuild 프로젝트 빌드 출력의 자세한 정도** 를 **보통**합니다.**확인**을 클릭합니다.  
  
4.  SimpleProject 프로젝트를 다시 빌드하십시오.  
...
## Visual Studio 템플릿 배포  
 Visual Studio 템플릿 경로 정보를 포함 하지 않습니다. 따라서 템플릿.zip 파일은 Visual Studio로 알려져 있는 위치에 배포 되어야 합니다. ProjectTemplates 폴더의 위치는 일반적으로 **\< % LOCALAPPDATA % \> \\Microsoft\\VisualStudio\\14.0Exp\\ProjectTemplates**합니다.  
  
 프로젝트는 팩터리를 배포 하려면 설치 프로그램에 관리자 권한이 있어야 합니다. Visual Studio 설치 노드 아래에서 템플릿을 배포: **...\\Microsoft Visual Studio 14.0\\Common7\\IDE\\ProjectTemplates**합니다.  
  
## Visual Studio 템플릿 테스트  
 Visual Studio 템플릿을 사용 하 여 프로젝트 계층 구조를 만들 것 있는지 확인 하 여 프로젝트 팩터리의 테스트 합니다.  
  
1.  Visual Studio SDK 실험적 인스턴스 다시 설정 합니다.  
  
     [!INCLUDE[win7](../debugger/includes/win7_md.md)]: 시작 메뉴의 찾기는 **Microsoft Visual Studio\/Microsoft Visual Studio SDK\/도구** 폴더를 선택한 다음 **Microsoft Visual Studio 실험적 인스턴스 다시 설정**합니다.  
  
     이후 버전의 Windows에서: 시작 화면에서 형식 **Microsoft Visual Studio \< 버전 \> 실험적 인스턴스 다시 설정**합니다.  
  
2.  명령 프롬프트 창이 나타납니다. 단어가 표시 되 면 `Press any key to continue`, enter 키를 누릅니다. 창을 닫은 후에 Visual Studio를 엽니다.  
  
3.  SimpleProject 프로젝트를 빌드하고 디버깅을 시작 합니다. 실험적 인스턴스가 표시 됩니다.  
...
## 프로젝트 형식 자식 노드 만들기  
 프로젝트 형식 노드를 자식 노드를 추가할 수는 **새 프로젝트** 대화 상자입니다.  예를 들어 SimpleProject 프로젝트 형식에 대 한 콘솔 응용 프로그램, 창 응용 프로그램, 웹 응용 프로그램 등에 대 한 자식 노드가 있을 수 있습니다.  
  
 자식 노드는 프로젝트 파일을 변경 하 고 \< ZipProject \> 요소에 \< OutputSubPath \> 자식을 추가 하 여 만들어집니다. 템플릿으로 빌드 또는 배포 하는 동안 복사 되 면 모든 자식 노드는 프로젝트 템플릿 폴더의 하위 폴더 됩니다.  
  
 이 섹션에는 SimpleProject 프로젝트 형식에 대 한 콘솔 자식 노드를 만드는 방법을 보여 줍니다.  
  
1.  \\Templates\\Projects\\ConsoleApp\\ \\Templates\\Projects\\SimpleProject\\ 폴더를 이름을 바꿉니다.  
  
2.  에 **속성** 창 \\Templates\\Projects\\ConsoleApp\\ 폴더에 모든 5 개의 파일을 선택 하 고 있는지 확인은 **빌드 작업** 로 설정 된 **ZipProject**합니다.  
  
3.  SimpleProject.vstemplate 파일에 닫는 태그 바로 전에 \< TemplateData \> 섹션의 끝에 다음 줄을 추가 합니다.  
...
5.  .Csproj 파일에서 \< OutputSubPath \> 각 ZipProject 요소를 추가 합니다. 이전 처럼 프로젝트를 업로드 하 고 프로젝트 파일을 편집 합니다.  
  
6.  \< ZipProject \> 요소를 찾습니다. 각 \< ZipProject \> 요소에 \< OutputSubPath \> 요소를 추가 하 고 콘솔 값을 지정 합니다. ZipProject  
...
7.  이 \< PropertyGroup \> 프로젝트 파일에 추가 합니다.  
...
## 프로젝트 형식 자식 노드를 테스트합니다.  
 테스트를 수정 된 프로젝트 파일 여부는 **콘솔** 에 자식 노드가 표시는 **새 프로젝트** 대화 상자.  
  
1.  실행 된 **Microsoft Visual Studio 실험적 인스턴스 다시 설정** 도구입니다.  
  
2.  SimpleProject 프로젝트를 빌드하고 디버깅을 시작 합니다. 실험적 인스턴스에서가 나타납니다.  
  
3.  에 **새 프로젝트** 대화 상자를 클릭 하는 **SimpleProject** 노드.**콘솔 응용 프로그램** 서식 파일에 표시 되어야는 **템플릿** 창입니다.  
  
4.  확장 된 **SimpleProject** 노드.**콘솔** 자식 노드가 표시 됩니다.**SimpleProject 응용 프로그램** 서식 파일에 계속 표시는 **템플릿** 창입니다.  
...
## 프로젝트 템플릿 매개 변수를 대체합니다.  
 [기본 프로젝트 시스템, 1 부](../extensibility/creating-a-basic-project-system-part-1.md) 덮어쓰는 방법을 보여 주었습니다는 `ProjectNode.AddFileFromTemplate` 템플릿 매개 변수를 대체 하는 기본 유형의 작업을 수행 하는 방법입니다. 이 섹션에는 보다 복잡 한 Visual Studio 템플릿 매개 변수를 사용 하는 방법을 보여 줍니다.  
  
 Visual Studio 템플릿을 사용 하 여 프로젝트를 만들 때의 **새 프로젝트** 프로젝트를 사용자 지정 대화 상자, 매개 변수가 바뀝니다 템플릿 문자열입니다. 템플릿 매개 변수는 시작 하 고 예를 들어 $time $에 달러 기호 끝나는 특수 토큰입니다. 다음 두 개의 매개 변수는 사용자 지정 서식 파일을 기반으로 하는 프로젝트에 사용 하도록 설정 하는 데 특히 유용 합니다.  
  
-   \[1\-10\] $GUID $는 새 Guid로 바뀝니다. 최대 10 개의 고유 Guid, 예를 들어, $guid1 $를 지정할 수 있습니다.  
  
-   $safeprojectname $는 사용자에 의해 제공 된 이름에서 **새 프로젝트** 대화 상자에서 안전 하지 않은 문자와 공백을 제거 하도록 수정 합니다.  
  
 템플릿 매개 변수 목록은 전체 참조 [템플릿 매개 변수](../ide/template-parameters.md)합니다.  사용자 고유의 사용자 지정 템플릿 매개 변수를 만들 경우 참조 [NIB: 방법: 사용자 지정 매개 변수를 템플릿에 전달](http://msdn.microsoft.com/ko-kr/5bc2ad11-84c7-4683-a276-e5e00d85d8fb)합니다.  
  
#### 프로젝트 템플릿 매개 변수를 대체  
  
1.  SimpleProjectNode.cs 파일에서 제거 된 `AddFileFromTemplate` 메서드.  
  
2.  \\Templates\\Projects\\ConsoleApp\\SimpleProject.myproj 파일에서 \< RootNamespace \> 속성을 찾은 다음 해당 값을 $safeprojectname $로 변경 합니다.  
  
    ```  
    <RootNamespace>$safeprojectname$</RootNamespace>  
    ```  
  
3.  \\Templates\\Projects\\SimpleProject\\Program.cs 파일에는 파일의 내용을 다음 코드로 바꿉니다.  
...
4.  SimpleProject 프로젝트를 빌드하고 디버깅을 시작 합니다. 실험적 인스턴스가 나타납니다.  
  
5.  새 SimpleProject 콘솔 응용 프로그램을 만듭니다. \(에 **프로젝트 형식** 창 **SimpleProject**합니다. 아래에서 **Visual Studio 설치 되어 있는 템플릿**, 선택, **콘솔 응용 프로그램**.\)  
...
## 프로젝트 속성 페이지 만들기  
 사용자가 보고 하 고 서식 파일을 기반으로 하는 프로젝트의 속성을 변경할 수 있도록 프로젝트 형식에 대 한 속성 페이지를 만들 수 있습니다. 이 섹션 구성 독립적 속성 페이지를 만드는 방법을 보여 줍니다. 이 기본 속성 페이지 속성 표를 사용 하 여 속성 페이지 클래스에서 노출 하는 공용 속성을 표시 합니다.  
 ...
 이 섹션에서 만드는 속성 페이지를 사용 하 여 변경 하 고 이러한 프로젝트 속성을 저장할 수 있습니다.  
...
1.  SimpleProjectPackage.cs 파일에서이 추가 `ProvideObject` 특성을 `SimpleProjectPackage` 클래스:  
    ...
    ```  
     ...
     속성 페이지 클래스를 등록 하는이 `GeneralPropertyPage` com  
  
2.  SimpleProjectNode.cs 파일에서 이러한 두 가지 재정의 된 메서드를 추가 `SimpleProjectNode` 클래스:  
...
3.  SimpleProject 프로젝트에 GeneralPropertyPage.cs 라는 클래스 파일을 추가 합니다.  
...
4.  다음 코드를 사용 하 여이 파일의 내용을 바꿉니다.  
    ...
    ```  
     ...
     `GeneralPropertyPage` 클래스 AssemblyName, OutputType, 및 RootNamespace 세 가지 공용 속성을 노출 합니다. AssemblyName에 set 메서드 없음, 있기 때문에 읽기 전용 속성으로 표시 됩니다. OutputType는 열거형된 상수 이므로 드롭다운 목록으로 표시 됩니다.  
  
     `SettingsPage` 기본 클래스를 제공 `ProjectMgr` 속성을 유지 하도록 합니다.`BindProperties` 메서드 `ProjectMgr` 지속형된 속성 값을 검색 하 고 해당 속성을 설정 합니다.`ApplyChanges` 메서드 사용 하 여 `ProjectMgr` 를 속성의 값을 가져와 프로젝트 파일에 보관 합니다. 이 속성이 설정 메서드 집합 `IsDirty` 속성을 유지 해야 함을 나타내려면 true로 합니다.  지 속성에는 프로젝트 또는 솔루션을 저장할 때 발생 합니다.  
  
5.  SimpleProject 솔루션을 빌드하고 디버깅을 시작 합니다. 실험적 인스턴스가 나타납니다.  
...
8.  프로젝트 노드를 마우스 오른쪽 단추로 클릭 **솔루션 탐색기**, 를 클릭 하 고 **속성**합니다.**속성 페이지** 대화 상자가 표시 됩니다.  
...
1.  에 **{1\>myconsoleapplication 속성 페이지** 변경 대화 상자는 **DefaultNamespace** 를 **MyApplication**합니다.  
  
2.  선택 된 **OutputType** 속성을 선택한 다음 **클래스 라이브러리**합니다.  
  
3.  클릭 **적용**, 를 클릭 하 고 **확인**합니다.  
  
4.  다시는 **속성 페이지** 대화 상자 및 변경 내용이 유지 되는지 확인 합니다.  
  
5.  Visual Studio의 실험적 인스턴스를 닫습니다.  
  
6.  실험적 인스턴스를 다시 엽니다.  
  
7.  다시는 **속성 페이지** 대화 상자 및 변경 내용이 유지 되는지 확인 합니다.  
