---
title: "사용자 지정 프로젝트 및 항목 템플릿 만들기 | Microsoft 문서"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
Visual Studio SDK에는 사용자 지정 프로젝트 템플릿을 만들고 사용자 지정 항목 템플릿을 프로젝트 템플릿이 포함 되어 있습니다. 이러한 템플릿에 일부 공통 매개 변수 대체 단어를 포함 하 고 zip 파일로 빌드합니다. 자동으로 배포 된 및 실험적 인스턴스에서 사용할 수 없는 항목입니다. Zip을 복사 해야 위치에 파일  
  
 서식 파일 만들기 템플릿을 크게 확장에 템플릿을 포함할 수 있습니다. 이 소스 파일에 버전 제어를 구현 하 고 하나의 VSIX 패키지에 서식 파일 프로젝트의 그룹을 빌드할 수 있습니다.  
...
1.  프로젝트 템플릿 프로젝트를 만듭니다. 프로젝트 템플릿을 찾을 수는 **새 프로젝트** 대화 상자에서 Visual Basic 또는 Visual C#에서 **확장성** 폴더입니다.  
  
     서식 파일 클래스 파일, 아이콘,.vstemplate 파일, ProjectTemplate.vbproj 또는 ProjectTemplate.csproj, 라는 편집 가능한 프로젝트 파일 및 일반적으로 다른 프로젝트 형식에 의해 생성 되는 일부 파일, 이러한 resources.resx 파일는 AssemblyInfo 파일 및 생성.settings 파일을 합니다. 적절 한 공통 매개 변수 대체를 포함 하는 각 코드 파일.  
  
2.  추가 하 고 프로젝트에 대 한 필요에 따라 프로젝트에서 항목을 제거 합니다. 편집 가능한 프로젝트 파일, AssemblyInfo 파일 또는.vstemplate 파일을 제거 하지 마십시오.  
  
3.  모든 추가 삭제를 반영 하도록.vstemplate 파일을 업데이트 합니다. [프로젝트](../extensibility/project-element-visual-studio-templates.md) 요소를 포함 해야는 [ProjectItem](../extensibility/projectitem-element-visual-studio-item-templates.md) 서식 파일에 포함 시킬 각 파일에 대 한 요소입니다.  
...
6.  프로젝트를 빌드합니다.  
     ...
     Visual Studio 템플릿이 포함 된.zip 파일을 만듭니다. 배포 되지 않은 및 되기 실험적 인스턴스에서 사용할 수 없습니다.  
...
6.  프로젝트를 빌드합니다.  
     ...
     Visual Studio 템플릿이 포함 된 압축된 파일을 만듭니다. 배포 되지 않은 및 되기 실험적 인스턴스에서 사용할 수 없습니다.  
...
3.  VSIX 프로젝트의 자산으로 프로젝트 서식 파일 프로젝트를 설정 합니다. .Vsixmanifest 파일을 엽니다. 이동 된 **자산** 탭을 클릭 **새로**합니다.  
...
5.  프로젝트 템플릿 프로젝트에 나열 된 프로젝트 템플릿이 표시 됩니다는 **새 프로젝트** 대화 (**파일 / 새 / 프로젝트**), Visual C# 또는 Visual Basic 노드. 템플릿 프로젝트 항목에 대 한 새 항목 추가 대화 상자에 나열 된 항목 템플릿을 표시 됩니다 (에 **솔루션 탐색기**, 프로젝트 노드를 선택 하 고 클릭 **추가 / 새 항목**).  
