---
title: "변경 된 값의 샘플 구현 | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
...
>  Visual Studio 2015의 식 계산기를 구현 합니다. 이러한 방식으로 사용 되지 않습니다. CLR 식 계산기를 구현 하는 방법에 대 한 정보를 참조 하십시오 [CLR 식 계산기](https://github.com/Microsoft/ConcordExtensibilitySamples/wiki/CLR-Expression-Evaluators) 및 [관리 되는 식 계산기 예제](https://github.com/Microsoft/ConcordExtensibilitySamples/wiki/Managed-Expression-Evaluator-Sample)합니다.  
 ...
 에 표시 되는 모든 로컬은 **지역** 창에는 [IDebugProperty2](../../extensibility/debugger/reference/idebugproperty2.md) 개체가 연결 되어 있습니다. 이 `IDebugProperty2` 개체에는 지역 변수의 이름, 값 및 형식이 포함 되어 있습니다. Visual Studio에서 호출 사용자의 로컬 값에서 변경 하는 경우 [SetValueAsString](../../extensibility/debugger/reference/idebugproperty2-setvalueasstring.md) 에서 로컬 메모리에 값을 업데이트 합니다. 이 예제에서는 로컬으로 표현 됩니다는 `CFieldProperty` 클래스를 구현 하는 `IDebugProperty2` 인터페이스입니다.  
  
> [!NOTE]
>  에 대 한 **조사식** 및 **간략 한 조사식** 식, 값 변경 나타내는 `CValueProperty` MyCEE 샘플의 클래스입니다. 그러나 구현의 `IDebugProperty2::SetValueAsString` 여기에 표시 된 것과 같습니다.  
  
 이 구현 `IDebugProperty2::SetValueAsString` 다음 작업을 수행 합니다.  
  
1.  값을 생성 하는 식을 계산 합니다.  
  
2.  연결 된 바인딩합니다 [IDebugField](../../extensibility/debugger/reference/idebugfield.md) 해당 메모리 위치에 개체를 생성 한 [IDebugObject](../../extensibility/debugger/reference/idebugobject.md) 개체입니다.  
...
## 관리 코드  
 구현인이 `IDebugProperty2::SetValueAsString` 관리 코드에서.  
...
## 관리 되지 않는 코드  
 구현인이 `IDebugProperty2::SetValueAsString` 관리 코드에서. 도우미 함수 `FieldCoerceValueType` \(표시 되지 않음\) 강제로 `VARIANT` 값은 형식 중 하나를 특정 형식 및 있는지는 `FieldSetValue` 처리할 수 있습니다.  
