...
#### 매개 변수  
 `pAddress`  
 \[in\] 디버그 주소를 표시 하는 [IDebugAddress](../../../extensibility/debugger/reference/idebugaddress.md) 인터페이스입니다.  
  
 `pSig`  
 \[in\] 검사 하는 배열입니다.  
  
 `dwSigLength`  
 \[in\] 길이 \(바이트\)는 `pSig` 배열입니다.  
  
 `ppField`  
 \[out\] 배열 형식을 나타내는 반환 된 [IDebugClassField](../../../extensibility/debugger/reference/idebugclassfield.md) 인터페이스.  
