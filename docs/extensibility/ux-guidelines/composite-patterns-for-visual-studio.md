...
-   차트는 사용자가 더 나은 결정을 내릴 수를 허용 합니까?  
...
2.  **실수:** 부동 프레 젠 터가 선택된 된 콘텐츠를 최대한 가깝게 배치 되지만 기존 내용의 레이아웃을 변경 되지 않습니다. 위에 떠 있는 콘텐츠 패널을 표시 하는 등 다양 한 전략에 고용 됨, 될 수는 선택한 기호에 공백을 사용할 수 있는 가장 가까운 합니다.  
...
|Shift + 클릭 또는 Ctrl 키를 포함 된 개체 및/또는 컨테이너에|기존 선택 또는 선택 영역 그룹을 클릭 한 개체를 추가합니다. 클릭 한 개체가 선택 그룹의 멤버인 이미 있으면 선택 그룹에서 제거 됩니다.|  
 ...
 포함 된 개체는 이전 섹션에 설명 된 대로 기본 선택 모델을 준수 해야 합니다. Windows Forms 디자이너의 유용성 테스트를 통해 사용자는 포함 된 개체에 대 한 원활한 액세스 (포함 개체에 의해 적용 됨) 하는 중간 단계 없이 필요 합니다.  
...
### <a name="overview"></a>개요  
 Visual Studio에서 각 소프트웨어 구성 요소는 일반적으로 자체 상태를 지 속성, 있지만 Visual Studio 자동으로 설정을 저장 일부 경우에 같은 창 크기와 위치 합니다. 다음 표에서 명시적 사용자가 필요 하거나 수행할 동작을 프로그래밍 하는 설정 및 자동으로 저장 된 설정이 조합입니다.  
  
|개체|저장을|저장 하는 경우|저장 위치|  
|------------|------------------|------------------|-------------------|  
|선택 가능한 개체 (예를 들어 코드의 줄)|코드의 줄에 중단점을 설정<br /><br /> 코드 줄과 연결 된 사용자 바로 가기|프로젝트를 저장 하는 경우| **사용자 옵션 (.suo)** 프로젝트 파일|  
|대화 상자|가 이동 된 경우 대화 상자에서의 위치<br /><br /> 대화 상자에서 마지막으로 사용 된 사용자 보기|대화 상자를 닫을 때<br /><br /> Visual Studio 세션의 종료 시점|메모리에<br /><br /> 레지스트리를 **HKEY_Current_User**|  
|창|크기 및 창 위치|창을 닫을 때<br /><br /> Visual Studio 모드 변경 될 때<br /><br /> Visual Studio 세션의 종료 시점| **사용자 옵션 (.suo)** 프로젝트 파일<br /><br /> 창 설정에 대 한 사용자 지정 옵션 파일|  
|문서|문서에서 현재 선택 영역<br /><br /> 문서 보기<br /><br /> 사용자가 방문 하는 마지막 여러 위치|문서가 저장 되는 경우| **사용자 옵션 (.suo)** 프로젝트 파일|  
|프로젝트|파일에 대 한 참조<br /><br /> 디스크에는 디렉터리에 대 한 참조<br /><br /> 다른 소프트웨어에 대 한 참조<br /><br /> 구성 요소<br /><br /> 프로젝트 자체에 대 한 상태 정보|프로젝트를 저장 하는 경우|프로젝트 파일|  
|솔루션|프로젝트에 대 한 참조<br /><br /> 파일에 대 한 참조|프로젝트 또는 솔루션이 저장 될 때| **솔루션 (.sln)** 파일|  
|설정 **도구 > 옵션**|키보드 사용자 지정<br /><br /> 도구 모음 사용자 지정<br /><br /> 색 구성표|경우는 **도구 > 옵션** 대화 상자를 닫습니다<br /><br /> Visual Studio 세션의 종료 시점|레지스트리를 **HKEY_Current_User**|  
  
 사용자가 알아서 수행 하 고, 작업이 수행 중인 경우 지정할지는 설정을 (세션 간에 레지스트리 설정으로)를 디스크에 저장 (세션) 하는 동안 메모리에 저장 되 고 있는지 여부를 프로젝트 또는 솔루션 파일 자체는 파트로 일부로 **솔루션 (.suo) 옵션** 파일 또는: 사용자 지정 설정에만 해당 소프트웨어 구성 요소 파일에 대해 알고 있습니다. 위의 표에서 설정을 저장할 수 있는 여러 이벤트를 보여 줍니다. 그러나 있는 상태를 저장 하려는 경우도 있습니다.  
  
-   사용자가 대화 상자 또는 창 내에서 위치를 변경 하는 경우  
  
-   사용자는 다른 창으로 포커스를 이동 하는 경우  
  
-   디버그 모드를 디자인 하는 사용자 간에 전환 될 때  
  
-   때 사용자가 로그 오프할 계정  
  
-   컴퓨터의 절전 모드로 전환 되거나 종료 때  
  
-   가 컴퓨터/하드 드라이브 다시 포맷 하 고 다시 설정 될 때  
  
### <a name="window-configurations"></a>창 구성  
 현재 도구 창의 목록을 및 정렬 되는 방식으로 구성 된 구성표를 창 구성을 개발 환경의 기본 표시. IDE (IDE 창)에 의해 관리 되는 windows에 대 한 레이아웃 정보는 사용자별로 유지 되므로 때 IDE에서 창 레이아웃으로 동일 하 게 표시 하는 사용자 시작 마지막은 Visual Studio 종료 되었습니다. 상태와 IDE 창의 위치는 사용자 지정 옵션 XML 형식 파일에에서 유지 됩니다. IDE에 로드 하는 패키지에 의해 만들어진 도구 창을 레지스트리에 해당 상태 정보를 유지 하 고 수도 사용자 당 되지 않을 수 있습니다.  
  
#### <a name="profile-specific-layouts"></a>프로필 관련 레이아웃  
 각 프로필에 특정 개발자 가상 사용자에 게 익숙한 방식으로 구성 되는 도구 창 레이아웃에 포함 됩니다 (Visual c + + 개발자는 **솔루션 탐색기** C# 개발자가이 표시 되도록 하는 동안에 IDE의 왼쪽에는 **솔루션 탐색기** 오른쪽). 프로필 관련 창 레이아웃을 사용자가 시작 시에는 프로필을 선택한 후에 로드 됩니다. 창 구성에 사용자가 변경한 내용을 유지 한 다음 함을 알고 있으면 고객의 환경에 가장 적합 한 창 레이아웃은 패키지 작성자에 확인 해야 합니다.  
  
##  <a name="a-namebkmktouchinputa-touch-input"></a><a name="BKMK_TouchInput"></a> 터치식 입력  
 사용자가 Microsoft 개발 제품을 사용 하 여 터치 장치에 점점 더 됩니다. 그러나 어렵게 터치 장치에 개발 도구를 사용 하 여 장벽이 있습니다. 사용자가 신뢰할 수 있는 고 정확 하 게 터치 환경을 제공 하기 위해 제품을 기대 합니다. 이 지침의 의도가 결정 통합 하 고 Visual Studio 및 관련된 제품 간에 일관 된 터치 환경을 조성 하는 터치 기능에 대 한 정보를 제공 합니다.  
  
### <a name="levels-of-experience"></a>수준의 경험이 있는  
 에서는 다음과 같은 수준의 경험 터치에 관심을 투자의 원하는 수준에 따라 터치 기능을 제공 하는 의사 결정 지침으로 사용 하는 데 사용 됩니다.  
  
-    **기본 경험** 작업 전체에서 배달 못 한 ends는 터치 기능을 제공 하려는 팀을입니다.  
