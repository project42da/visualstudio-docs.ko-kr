---
title: "MSBuild 오류 MSB3121 | Microsoft Docs"
ms.custom: ""
ms.date: "11/17/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: ""
ms.topic: "article"
f1_keywords: 
  - "GenerateManifest.FileAssociationMissingAttribute"
helpviewer_keywords: 
  - "MSB3121"
ms.assetid: c1e83a2a-515f-412d-b8b7-4821e510a923
caps.latest.revision: 7
caps.handback.revision: 7
author: "mikeblome"
ms.author: "mblome"
manager: "douge"
---
# MSBuild 오류 MSB3121
**MSB3121: 응용 프로그램 매니페스트의 파일 연결 요소에 필요한 특성인 확장명, 설명, progid 또는 기본 아이콘 중 하나 이상이 없습니다.**  
  
 [ClickOnce 배포 매니페스트](../deployment/clickonce-deployment-manifest.md)에는 네 특성에 대한 값이 모두 있어야 합니다.  
  
### 이 오류를 해결하려면  
  
-   각 [ClickOnce 배포 매니페스트](../deployment/clickonce-deployment-manifest.md) 특성을 올바른 값으로 설정합니다.  
  
## 참고 항목  
 [프로젝트 디자이너, 게시 페이지](../ide/reference/publish-page-project-designer.md)   
 [ClickOnce 응용 프로그램 매니페스트](../deployment/clickonce-application-manifest.md)