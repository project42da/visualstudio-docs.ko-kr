...
'class'는 'member' 인터페이스 멤버를 구현하지 않습니다.  
 ...
 [클래스](/dotnet/csharp/language-reference/keywords/class)가 [인터페이스](/dotnet/csharp/language-reference/keywords/interface)에서 파생되었지만 해당 클래스가 인터페이스 멤버를 하나 이상 구현하지 않았습니다. 클래스는 클래스가 파생된 인터페이스의 모든 멤버를 구현하거나 `abstract`로 선언되어야 합니다.  
