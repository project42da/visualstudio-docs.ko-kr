---
title: "Internal error processing pattern. | Microsoft Docs"
ms.custom: ""
ms.date: "11/16/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: ""
ms.topic: "article"
f1_keywords: 
  - "vs.message.VS_E_RE_INTERNALERROR"
  - "vs.message.0x800A00B8"
ms.assetid: a4b176fb-3fae-40d0-8b4c-088d85be2d1e
caps.latest.revision: 6
caps.handback.revision: 6
author: "mikeblome"
ms.author: "mblome"
manager: "douge"
---
# Internal error processing pattern.
이 오류는 검색 문자열에 정규식이 포함되었지만 이 정규식 구문에 오류가 있는 경우 발생합니다.  
  
### 이 오류를 해결하려면  
  
1.  올바른 정규식 구문을 사용하여 검색 문자열을 다시 입력합니다.  자세한 내용은 [정규식](../ide/using-regular-expressions-in-visual-studio.md)을 참조하십시오.  
  
## 참고 항목  
 [텍스트 찾기 및 바꾸기](../ide/finding-and-replacing-text.md)   
 [Visual Studio에서 정규식 사용](../ide/using-regular-expressions-in-visual-studio.md)   
 [Wildcards \(Visual Studio\)](http://msdn.microsoft.com/ko-kr/9745c56f-0b73-44e1-b393-c17e39670c26)