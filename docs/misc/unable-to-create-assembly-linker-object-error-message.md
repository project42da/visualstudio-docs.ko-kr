...
[!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 컴파일러가 어셈블리 링커\(Al.exe, Alink라고도 함\)를 호출하여 매니페스트를 사용해 어셈블리를 생성합니다. 작업 개체를 만드는 중에 링커가 오류를 보고했습니다. 이로 인해 가져오기 파일 호출 실패 또는 메타데이터 파일에 대한 참조를 생성하는 문제가 발생할 수 있습니다.  
