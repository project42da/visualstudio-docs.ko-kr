...
# URL 선택기 대화 상자(Visual Studio에서 SharePoint 개발)
 ...
 속성 설정을 위해 파일을 선택할 수 있는 옵션이 있는 경우에 이 대화 상자가 표시됩니다.  이 대화 상자는 **속성** 창의 여러 속성 옆에 있는 줄임표 단추\(![ASP.NET 모바일 디자이너 줄임표](~/docs/sharepoint/media/mwellipsis.gif "ASP.NET 모바일 디자이너 줄임표")\)를 선택하여 열 수 있습니다.  줄임표 단추는 디자이너의 **소스** 뷰에서 특정 특성에 값을 할당하면 IntelliSense 프롬프트로 나타나기도 합니다.  
