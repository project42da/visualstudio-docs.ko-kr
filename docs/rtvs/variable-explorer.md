...
추가 조작 및 분석을 위해 일반적으로 세션 변수를 CSV로 내보내는 것이 좋습니다. 이 작업을 수행하려면 변수 탐색기에서 각 노드 옆에 있는 작은 Excel 아이콘(![Excel 내보내기 아이콘](media/variable-explorer-excel-icon.png))을 사용하거나 항목을 마우스 오른쪽 단추로 클릭하고 **CSV 앱에서 열기**를 선택합니다. 아이콘을 선택하면 데이터가 `%userprofile%\Documents\RTVS_CSV_Exports` 폴더의 새 CSV 파일에 기록되고 해당 파일이 시작됩니다. 파일은 `.csv` 확장과 연결된 응용 프로그램에서 열립니다.
