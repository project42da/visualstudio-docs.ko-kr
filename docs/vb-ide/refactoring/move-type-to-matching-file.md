---
title: "일치 하는 파일-리팩터링 (Visual Basic)으로 형식을 이동 | Microsoft Docs"
ms.custom: 
ms.date: 11/17/2016
ms.reviewer: 
ms.suite: 
...
---
# <a name="move-type-to-a-matching-file-in-visual-basic"></a>Visual Basic에서 일치 하는 파일 형식을 이동합니다
...
1. 다음으로, 다음 중 하나를 수행 합니다.
   * **키보드**
     * 키를 눌러 **Ctrl +.** 트리거에 **빠른 작업 및 리팩터링** 메뉴를 선택 **종류를 이동 *TypeName*.vb** 미리 보기 창 팝업에서 위치 *TypeName* 선택한 형식의 이름입니다.
   * **마우스**
     * 코드를 마우스 오른쪽 단추로 클릭을 선택는 **빠른 작업 및 리팩터링** 메뉴와 선택 **종류를 이동 *TypeName*.vb** 미리 보기 창 팝업에서 위치  *TypeName* 선택한 형식의 이름입니다.
