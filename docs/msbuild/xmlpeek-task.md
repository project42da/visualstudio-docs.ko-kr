...
XPath 쿼리에 지정된 대로 XML 파일의 값을 반환합니다.  
...
|`Result`|선택적 <xref:Microsoft.Build.Framework.ITaskItem>`[]` 출력 매개 변수입니다.<br /><br /> 이 작업에서 반환한 결과를 포함합니다.|  
|`XmlContent`|선택적 `String` 매개 변수입니다.<br /><br /> 문자열로 XML 입력을 지정합니다.|  
