---
title: "JavaScript에서 Windows 런타임 이벤트 처리 | Microsoft Docs"
ms.custom: ""
ms.date: "01/18/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "JavaScript, Windows 런타임 이벤트"
  - "Windows 런타임 이벤트[JavaScript]"
...
---
# JavaScript에서 Windows 런타임 이벤트 처리
Windows 런타임 이벤트는 C\+\+ 또는 .NET Framework에서 표현되는 방식과 다른 방식으로 JavaScript에서 표현됩니다.  Windows 런타임 이벤트는 클래스 속성이 아니며 클래스의 `addEventListener` 및 `removeEventListener` 메서드에 전달되는 문자열 식별자로 표현됩니다.  예를 들어, 문자열 "positionchanged"를 `Geolocator.addEventListener` 메서드에 전달하여 [Geolocator.PositionChanged](http://msdn.microsoft.com/library/windows/apps/xaml/windows.devices.geolocation.geolocator.positionchanged.aspx) 이벤트의 이벤트 처리기를 추가할 수 있습니다.  
