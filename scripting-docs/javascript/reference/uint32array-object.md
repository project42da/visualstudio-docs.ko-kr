...
---
# Uint32Array 개체
32비트 부호 없는 정수 값에 대해 형식화된 배열입니다.  콘텐츠가 0으로 초기화됩니다.  요청된 바이트 수를 할당할 수 없으면 예외가 발생합니다.  
...
## 매개 변수  
 `uint32Array`  
 필수입니다.  **Uint32Array** 개체가 할당된 변수 이름입니다.  
 ...
 `byteOffset`  
 선택 사항  버퍼의 시작 지점에서 Uint32Array를 시작할 오프셋\(바이트\)을 지정합니다.  
...
|상수|설명|  
|--------|--------|  
|[BYTES\_PER\_ELEMENT 상수](../../javascript/reference/bytes-per-element-constant-uint32array.md)|배열에서 각 요소의 크기\(바이트\)입니다.|  
...
## 메서드  
 다음 표에서는 `Uint32Array` 개체의 메서드를 보여줍니다.  
