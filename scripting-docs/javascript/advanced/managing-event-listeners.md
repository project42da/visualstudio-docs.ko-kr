...
## 이벤트 수신기 및 개체 범위  
 다음 코드 예제에서는 `dataObjFactory()` 함수 호출 시 메모리 누수를 발생시킬 수 있는 코드를 보여 줍니다.  이 코드에서는 새 데이터 개체를 만들 때마다 데이터 개체에 대해 이벤트 수신기가 등록됩니다.  `dataReady()` 이벤트 처리기에서 현재 데이터 개체를 사용할 수 있도록 `addEventListener`에서 [bind 메서드\(Function\)](../../javascript/reference/bind-method-function-javascript.md)가 사용됩니다.  
...
```  
 ...
 마지막으로 바인딩된 함수의 저장된 참조\(`handlerRef`\)에 대해 `removeEventListener`를 호출해야 합니다.  수정된 `dataObjFactory`의 코드는 다음과 같습니다.  
